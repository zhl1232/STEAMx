...
import type { ProfileNextAction, ProfileNextActionVariant } from '@/lib/profile/next-action'
...
function spotIconForVariant(variant: ProfileNextActionVariant): 'growth-quest' | 'exploring-map' | null {
  if (variant === 'growth' || variant === 'reward') return 'growth-quest'
  if (variant === 'exploring' || variant === 'vacuum') return 'exploring-map'
  return null
}
...
export function ProfileNextActionCard({
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        ...
        {action.secondaryHref && action.secondaryLabel ? (
          <Link
            ...
            className="inline-flex min-h-11 shrink-0 items-center gap-0.5 text-xs font-bold text-[hsl(var(--brand-blue))] transition hover:text-[hsl(var(--brand-blue)/0.82)]"
          >
      ...
      <div
        className={cn(
          'mt-3 grid grid-cols-[48px_minmax(0,1fr)_auto] items-center gap-3',
          !isMobile &&
      ...
      >
        ...
        <div className="min-w-0">
          <p className="line-clamp-1 text-sm font-semibold leading-snug text-foreground">{action.title}</p>
