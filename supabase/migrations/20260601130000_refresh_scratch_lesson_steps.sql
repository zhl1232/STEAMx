-- 强制刷新 Scratch 全部 8 课步骤为「配色积木标签」版（UPDATE，对已存在课时生效）。
-- 背景：上一版 20260601120000 对新增课时用 INSERT ... WHERE NOT EXISTS（幂等），
-- 已存在的课时不会被更新；且同名 migration 不会重跑。故用本文件按 title 强制 UPDATE。
-- 步骤文案用标记 [[cat:x]] / [[block:x|文字]]，由前端 lesson-rich-text.tsx 渲染为
-- Scratch 配色的分类圆点与积木块（分类英文 key 统一映射中文名，杜绝写错「动作/运动」）。
DO $$
DECLARE
  v_course_id bigint;
BEGIN
  ...
  SET sort_order = 1, duration_minutes = 25,
      content = '{"summary":"第一次见面！让小猫动起来、说句话，认识 Scratch 的舞台和积木。","tutorialDeckId":"intro-move-sayhello"}'::jsonb,
      steps = '[
  ...
  WHERE course_id = v_course_id AND title = '认识 Scratch';

  -- [2] 动画故事 —— tell-a-story
  UPDATE public.course_lessons
  SET sort_order = 2, duration_minutes = 40,
      content = '{"summary":"让角色动起来、说话，创作你的第一个 Scratch 作品。","tutorialDeckId":"tell-a-story"}'::jsonb,
      steps = '[
  ...
  UPDATE public.course_lessons
  SET sort_order = 3, duration_minutes = 35,
      content = '{"summary":"制作会动、能互动的电子贺卡，掌握循环和事件。","tutorialDeckId":"animate-a-name"}'::jsonb,
      steps = '[
  ...
  -- [4] 想象世界 —— imagine
  UPDATE public.course_lessons
  SET sort_order = 4, duration_minutes = 40,
      content = '{"summary":"放飞想象：选场景、加角色，让它飞、变身、发声，创造属于你的小世界。","tutorialDeckId":"imagine"}'::jsonb,
      steps = '[
  ...
  WHERE course_id = v_course_id AND title = '想象世界';

  -- [5] 追逐游戏 —— chase-game
  UPDATE public.course_lessons
  SET sort_order = 5, duration_minutes = 45,
      content = '{"summary":"做一个用方向键抓目标的小游戏，掌握按键控制、碰撞侦测和计分。","tutorialDeckId":"chase-game"}'::jsonb,
      steps = '[
  ...
  WHERE course_id = v_course_id AND title = '追逐游戏';

  -- [6] 弹球游戏 —— pong-game
  UPDATE public.course_lessons
  SET sort_order = 6, duration_minutes = 50,
      content = '{"summary":"综合运用移动、碰撞、变量和条件，完成可玩的弹球游戏。","tutorialDeckId":"pong-game"}'::jsonb,
      steps = '[
  ...
  -- [7] 飞行闯关 —— make-it-fly
  UPDATE public.course_lessons
  SET sort_order = 7, duration_minutes = 50,
      content = '{"summary":"用鼠标操控角色飞行、躲避并收集道具，综合运用坐标、循环和变量。","tutorialDeckId":"make-it-fly"}'::jsonb,
      steps = '[
  ...
  WHERE course_id = v_course_id AND title = '飞行闯关';

  -- [8] 音乐创作 —— music
  UPDATE public.course_lessons
  SET sort_order = 8, duration_minutes = 35,
      content = '{"summary":"用音乐扩展演奏音符和鼓点，编出属于自己的一小段旋律。","tutorialDeckId":"music"}'::jsonb,
      steps = '[
  ...
  WHERE course_id = v_course_id AND title = '音乐创作';
