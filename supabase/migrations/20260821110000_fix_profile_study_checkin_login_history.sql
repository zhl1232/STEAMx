-- Fix profile study check-in streaks by including the historical daily login ledger.
-- profiles.last_check_in only stores the latest login date, so it cannot represent
-- consecutive login days on its own. daily_check_in already writes one xp_logs row
-- per login day; use its created_at as the historical login-day source.
...
BEGIN
  WITH qualifying_days AS (
    -- Keep the latest profile value as a compatibility fallback for old data.
    SELECT DISTINCT p.last_check_in AS event_date
    FROM public.profiles p
    WHERE p.id = target_user_id
      AND p.last_check_in IS NOT NULL

    UNION

    -- daily_check_in writes one immutable daily_login ledger row per new day.
    SELECT DISTINCT (dl.created_at AT TIME ZONE 'Asia/Shanghai')::date AS event_date
    FROM public.xp_logs dl
    WHERE dl.user_id = target_user_id
      AND dl.action_type = 'daily_login'
