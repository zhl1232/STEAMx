-- 探索页「本周热门」：按近 7 天项目互动加权排序
-- 互动项：项目点赞、项目评论/回复、探索记录提交、投币（项目/记录）、记录评论
...
COMMENT ON FUNCTION public.get_weekly_hot_project_rankings IS
  '近 7 天互动热度：点赞×3、评论/回复×2、探索记录×5、投币按枚数、记录评论×2';

GRANT EXECUTE ON FUNCTION public.get_weekly_hot_project_rankings(
  timestamptz, integer, integer, text, text, integer, integer, text[], text, text[]
) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_weekly_hot_project_rankings(
  timestamptz, integer, integer, text, text, integer, integer, text[], text, text[]
) TO anon;
...
CREATE INDEX IF NOT EXISTS idx_completed_projects_completed_at ON public.completed_projects (project_id, completed_at DESC);
CREATE INDEX IF NOT EXISTS idx_coin_logs_tip_created_at ON public.coin_logs (created_at DESC)
