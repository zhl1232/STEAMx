-- Align get_user_stats_summary.speciesObserved with TS observation progress:
-- consensus species first; otherwise active AI identifications with confidence >= 0.8.
...
-- Allow users to clear their own playground cloud stats
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public'
      AND tablename = 'playground_stats'
      AND policyname = 'Users can delete own playground stats'
  ) THEN
    CREATE POLICY "Users can delete own playground stats"
      ON public.playground_stats
      FOR DELETE
