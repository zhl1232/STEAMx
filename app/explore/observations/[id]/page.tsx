...
export default async function ObservationDetailPage({ params }: ObservationDetailPageProps) {
  ...
  return (
    <div className="container mx-auto max-w-4xl px-4 py-10 md:py-14">
      <MobilePageHeader
        title="观察记录详情"
        fallbackHref="/explore/observations"
        className="-mx-4 -mt-10 mb-4 md:hidden"
      />
      <MobileBackButton fallbackHref="/explore/observations" className="hidden md:block" />

      <div className="mt-4 rounded-3xl border bg-card px-6 py-8 shadow-sm md:px-10 md:py-10">
        <h1 className="text-3xl font-bold tracking-tight">观察记录 #{observation.id}</h1>
        ...
        {observation.notes && (
          <div className="mt-6 rounded-2xl border bg-muted/20 p-5">
            ...
            <p className="mt-3 text-sm leading-6 text-muted-foreground">{observation.notes}</p>
          </div>
        )}
        ...
        <section className="mt-8 rounded-2xl border p-5">
          ...
          <div className="mt-4 space-y-4">
            {observation.species.map((item) => (
              <div key={item.speciesId} className="rounded-2xl border bg-muted/20 p-4">
                ...
                {item.behaviorTags.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {item.behaviorTags.map((tag) => (
                      <span key={`${item.speciesId}-${tag}`} className="rounded-full bg-background px-3 py-1 text-xs font-medium text-muted-foreground">
                        {tag}
                      </span>
                    ))}
                  </div>
