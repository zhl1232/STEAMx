...
import {
  EXPLORE_FOR_YOU_LIMIT,
  getExploreForYouRecommendations,
...
function parseMode(value: string | null): HomepageRecommendationMode | undefined {
  return value === "popular-fallback" ? value : undefined;
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const limit = Math.min(
      EXPLORE_FOR_YOU_LIMIT,
      Math.max(1, parsePositiveInteger(searchParams.get("limit"), EXPLORE_FOR_YOU_LIMIT)),
    );
    ...
    const payload = await getExploreForYouRecommendations({
      limit,
      offset,
      mode,
      excludeIds,
    ...
    logger.error("Error in GET /api/explore/recommendations", { error });
    return NextResponse.json(
      { error: "Failed to fetch explore recommendations" },
      { status: 500 },
    );
  }
}
