...
export async function uploadDirectory(client, {
  ...
  const files = await listLocalFiles(absLocalDir)

  console.log(`Uploading ${files.length} files from ${absLocalDir} → oss:${publicPathPrefix}/`)
  ...
  console.log(`✓ Uploaded ${files.length} files to oss:${publicPathPrefix}/`)
  return files.length
}
