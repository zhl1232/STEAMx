...
describe("buildPageMetadata", () => {
  ...
  it("leads with high-volume search terms and avoids Valve steam教育", () => {
    expect(DEFAULT_SEO_KEYWORDS).toEqual(
      expect.arrayContaining(["少儿编程", "积木", "自然观察"]),
    );
    ...
    const metadata = buildPageMetadata({
      title: "少儿编程 · 积木 · 自然观察",
      description: "STEAMX（史迪姆）给孩子做少儿编程、积木课和自然观察。动手搭、去观察，把项目做成作品。",
      path: "/",
    });

    expect(metadata.title).toBe("少儿编程 · 积木 · 自然观察");
    expect(metadata.description).toContain("STEAMX（史迪姆）给孩子做少儿编程、积木课和自然观察");
