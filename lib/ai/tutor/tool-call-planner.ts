...
function buildPlannerPrompt(input: PlannerInput) {
  ...
  return [
    ...
    '必须根据用户消息的完整语义和当前场景判断，不要用固定关键词、正则表达式或短语白名单筛选意图；自然表达、代词和上下文同样有效。',
    '只输出一行 JSON，不要输出解释。',
...
export async function planTutorToolDecision(input: PlannerInput) {
  const availableTools = getAvailableTutorTools(input)
  // This only avoids invoking the planner when the current page has no
...
export function shouldPlanTutorToolDecision(input: PlannerInput) {
  const availableTools = getAvailableTutorTools(input)
  // Capability check only; do not turn this function into a keyword intent
  // classifier. The planner must see ordinary questions and can return no-op.
