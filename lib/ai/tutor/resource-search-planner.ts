...
const RESOURCE_PLANNER_PROMPT = [
  ...
  '如果 shouldSearch=true，从学生原话中提取 1-4 个最能定位资源的主题短语，例如作品名、物体、技能、学科、玩法或材料。不要把“课程/项目/有没有/推荐”等意图词当作主题短语；如果只是泛问“有什么课程/项目”，queries 可以为空。',
...
function fallbackPlan(message: string): TutorResourceSearchPlan {
  return {
    status: 'fallback',
    // 规划器不可用时仍保留一次受限检索机会，但绝不把失败当成“站内没有”。
    shouldSearch: true,
    queries: [safeQuery(message)].filter(Boolean),
...
export async function planTutorResourceSearch(
  ...
  if (!message.trim()) return { ...fallback, shouldSearch: false, queries: [] }
