...
const RESOURCE_PLANNER_PROMPT = [
  ...
  '判定优先级：如果学生明确是在问当前课时、当前步骤、这一步积木怎么操作或怎么拼，且没有明确要求另一门课程/项目，必须 shouldSearch=false；这是当前学习指导，不是资源检索。除此之外，只要有一点可能是在找站内资源就 shouldSearch=true；拿不准时也选 true，避免漏掉自然表达。明确的知识问答、题目讲解、闲聊且没有在找站内资源时也选 false。',
