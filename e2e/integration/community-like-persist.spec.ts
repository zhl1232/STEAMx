...
import { deleteDiscussionByTitle, deleteUserByEmail } from './helpers/supabase-admin'
...
test.describe('社区回复点赞状态', () => {
  test('刷新后仍显示已点赞', async ({ page, browser }) => {
    ...
    const authorIsDisposable = !process.env.E2E_USER_EMAIL
    ...
    try {
      ...
      await deleteDiscussionByTitle(discussionTitle).catch(() => undefined)
      await deleteUserByEmail(likerEmail).catch(() => undefined)
      if (authorIsDisposable) {
        await deleteUserByEmail(authorEmail).catch(() => undefined)
      }
    }
  })
})
