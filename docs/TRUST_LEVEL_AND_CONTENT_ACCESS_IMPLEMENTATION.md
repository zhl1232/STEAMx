...
| `app/api/settings/age-confirmation/route.ts` | 读取和提交本人年龄确认 |
| `app/settings/security/page.tsx` | 提供本人确认操作和状态反馈 |
| `lib/context/login-prompt-context.tsx` | 处理 `AGE_CONFIRMATION_REQUIRED` 的设置页跳转、原请求暂存、确认后重试与返回 |
| `lib/utils/http.ts` | 解析稳定的 API 错误 `code/details`，校验年龄确认跳转地址 |
...
普通 profile PATCH 不接受年龄或限制字段。数据库触发器会保护这两个字段，年龄确认只能通过 `confirm_my_age()`，限制状态由 service/admin 流程写入。
...
主要写入入口在收到 `AGE_CONFIRMATION_REQUIRED` 后，会通过 `runAfterAgeConfirmation()` 暂存原始请求和当前页面，跳转 `/settings/security?section=age-confirmation`。设置页调用本人确认 RPC 成功后重发请求，并返回原页面；重发仍被门禁拦截时不会覆盖新的待处理请求。
...
- 迁移会将旧的 `profiles.age_confirmed_at` 清空，因为旧值来自注册/短信流程，不能证明本人确认。
- 最新 `handle_new_user()` 不再写年龄确认时间。
...
- 未确认用户执行投稿、评论、发帖、私信得到 `AGE_CONFIRMATION_REQUIRED`。
- 完成本人确认后上述接口放行。
