...
PBL（Project-Based Learning）挑战赛系统是平台的核心学习模块之一，将原本简单的"报名计数器"改造为一个 **PBL 驱动的双轨学习任务系统**。系统支持两种挑战类型——**限时挑战**（竞赛排名）和**长期挑战**（自主学习任务），并集成了多维评价、STEAM 雷达图、PBL 反思迭代等深度学习功能。
