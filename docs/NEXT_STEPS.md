...
- [x] **核心 MVP：融入 PBL 字段与项目反思** ✅ 已在 PBL 挑战系统中实现
...
- [x] **PBL 任务中心 (Mission Hub)** ✅ 已在 PBL 挑战系统中实现
...
- [x] **记录“试错迭代”过程 (Devlog)** ✅ 已在 PBL 挑战系统中实现
...
- [~] **同行评审 (Peer Review) 与多维度评价** （部分完成）
  ...
  - ✅ 评价雷达：已引入四维打分机制（创意性、实用性、技术难度、反思深度），集成于挑战作品互评。
...
> 平台定位：兴趣驱动的 STEAM 探索，不是课后辅导。内容参考课本但不标注课本版本和年级，用难度星级自然分层，标签标注知识领域。面向 6-16 岁。
...
| 主分类 | 调整前子分类 | 调整后子分类 |
|--------|-------------|-------------|
| **科学** | 物理实验、化学实验、~~生物观察~~、~~天文地理~~ | 物理实验、化学实验、**动物观察**、**植物观察**、**地球与天空** |
| 技术 | 编程入门、电子制作、机器人、3D打印 | 不变 |
| 工程 | 机械结构、桥梁建造、简易机器、模型制作 | 不变 |
| 艺术 | 绘画、手工、雕塑 | 不变 |
| 数学 | 几何探索、数学游戏、逻辑谜题 | 不变 |

- 细粒度通过 `tags` 字段实现（如 `鸟类`、`昆虫`、`岩石`、`天气`）。
- 已删除孤岛页面 `color-lab` 和 `pixel-art`（无入口，非数据库项目）。

### 内容来源与规划

三大来源，第一批目标 **~50 个项目**：

1. **课本知识关联**（~20 个）：参考小学至初中科学/物理/化学/数学课本核心知识点，转化为动手项目。不标注课本版本，仅用标签标注知识领域（如「浮力」「光学」「对称」）。
2. **自然观察系列**（~20 个）：覆盖动物观察（鸟类、昆虫、海鲜河鲜）、植物观察（树木、野花、蔬菜、园林花卉、水果干果）、地球与天空（矿物岩石、云和天气）。
   - 树木物种库仍需补 6 个封面图：黑桦（`betula-dahurica`）、鹅耳枥（`carpinus-turczaninowii`）、毛白杨（`populus-tomentosa`）、秋火焰银红槭（`acer-freemanii-autumn-blaze`）、枳（`citrus-trifoliata`）、花椒（`zanthoxylum-bungeanum`）。
3. **兴趣拓展**（~10 个）：编程、逻辑谜题、3D 打印、电子制作等。

### 内容生成流程

1. [x] 规划分类体系与内容策略（本节）
2. [ ] 生成所有分类下的项目标题清单（含分类、子分类、标签、难度星级）
3. [ ] 逐批生成每个项目的详细内容（介绍、材料、步骤），写入 SQL 迁移
4. [ ] 更新前端分类配置 `lib/config/categories.ts` + 数据库 `sub_categories` 迁移
5. [ ] 管理员通过后台补充封面图片并微调内容
...
- [x] **[#2] 挑战体系不完整（需整体规划）** ✅ 已通过 PBL 双轨挑战系统完整实现
