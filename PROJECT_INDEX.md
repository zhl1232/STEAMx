...
| 路由 | 入口文件 | 功能说明 |
|------|----------|----------|
| `/` | `app/page.tsx` | 首页 — Hero 主 CTA 收口到主线积木课（`MAINLINE_ENTRY_HREF`），次级按钮才是探索项目；**移动端首屏必须有主 CTA**（改版前按钮容器是 `hidden md:flex`，手机首屏一个按钮都没有，第一个可点元素要滚动才出现）；紧凑的「从这里开始」分类入口承接新用户行动；主体不再重复探索页热门项目，改为统一作品「大家的新作品」与最近公开观察「自然新发现」，桌面双栏、移动端顺序滚动；继续保留移动端自然观察/排行榜快捷入口、桌面社区动态与本周挑战（挑战卡读取当前 `active` PBL，优先限时挑战，卡片进入 `/pbl/[id]`，「查看全部」进入 `/create`） |
