...
| `/courses/.../lessons/[lessonId]` | `app/courses/[courseId]/lessons/[lessonId]/` | 课时学习页（侧栏步骤 + 可选学习目标/教师引导 + 按 `lesson_type` 切换工作区：Scratch 编辑器 / 大颗粒积木 3D 搭建预览 / 游乐场实训导学；3D 用 three.js `LDrawLoader` 加载自托管 `.mpd`，`0 STEP` 驱动分步显隐；学前大颗粒模型源由 `scripts/ldraw-preschool-model-geometry.test.ts` 守护不悬空、不穿模、不斜插；playground 课时把游乐场游戏包成导学课，右侧「去实战」按钮跳到对应 `/playground/*` 游戏页；playground 课时在移动端用单栏：隐藏左侧 `LessonSidebar` 步骤列表，由 `PlaygroundWorkspace` 承载讲解 + 紧凑进度条 + 上一步/下一步/完成 + 底部「去实战」按钮，桌面端仍保留双栏） |
...
| assets | `api/assets/` | 本地开发用受限静态资源代理；代理已迁移到 OSS 的 `/birds`、`/insects`、`/trees`、`/fruits`、`/projects`、**`/courses`**（课件 slides/PDF/视频/成品图/LDraw）资源。本地默认经代理带生产 Referer 拉取 OSS；**OSS 404 时开发环境回退 `public/` 同名路径**（课件先落本地、后传 OSS 时 dev 仍可用）；生产环境直连 `NEXT_PUBLIC_ASSETS_BASE_URL`；非生产设置 `NEXT_PUBLIC_ASSETS_DISPLAY_MODE=direct` 可绕过代理直连排查 |
...
| `ldraw-models/gen-eiffel.mjs` | 用 4 重对称 + 逐层 `topY` 跟踪生成埃菲尔铁塔 `eiffel-tower.ldr`（13 步对照搭建说明 PDF），内置悬空/同层穿模几何自检；改模型后重跑此脚本再 `pack-ldraw-model.mjs` 打包 |
| `ldraw-models/gen-3-bao-jian.mjs` | 生成 3+ 第 2 课「宝剑」`3-bao-jian.ldr`（9 步对照课件 slide 07–15），自动堆叠 + 几何自检；用法 `node scripts/ldraw-models/gen-3-bao-jian.mjs` |
| `export-courseware-slides.mjs` | 批量把 `C:\Users\Administrator\Documents`（WSL 路径 `/mnt/c/Users/Administrator/Documents`）下的 3+/4+/5+ 课件 PPTX 导出到 `public/courses/<grade-pinyin-slug>/slides/slide-*.png`：先从 PPTX 包统计真实页数，跳过已完整目录，渲染到临时目录成功后再替换目标 `slide-*`，并刷新已有 `scripts/courseware/<slug>.json` 的 `slideImageUrls`；支持 `--dry-run`、`--force`、`--only=`、`--limit=`、`--upload`（只上传 slides 到 `oss:courses/<slug>/slides/`，默认覆盖旧对象以修复早期单页导出）、`--skip-existing-upload`；LibreOffice 在沙箱/无图形后端环境可能失败，需在可用图形后端或沙箱外执行 |
...
| `check-courseware-oss.mjs` | 用 OSS `headObject` 核对 300 课 `slides`/`pdf`/`video`/`finished` 是否已上传（绕过 CDN 防盗链）；输出汇总 stats |
...
| `pack-ldraw-model.mjs` | 把 `scripts/ldraw-models/*.ldr` 递归抓取依赖打包成单个自托管 `.mpd` + `LDConfig.ldr` 到 `public/courses/ldraw/`（大颗粒积木 3D 课用，零件库 CC BY / CCAL）；打包时优先复用已提交 `.mpd` 内联依赖作为本地缓存，再缺失时联网抓取；用法 `node scripts/pack-ldraw-model.mjs <source.ldr> <outName>` |
