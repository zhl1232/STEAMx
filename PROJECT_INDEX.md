...
| `lib/works/` | `types.ts`, `capability.ts`, `data.ts`, `submission.ts` | 统一作品领域层：判断课时是否支持作品、读取/映射作品、提交课程作品、按来源补齐课程或项目信息；`getUserWorks()` 把**四条产出路径**汇总成一份「我的作品」——项目最终作品、课程课时作品、PBL 挑战作品（`challenge_submissions`）、自然观察（`observation_events`），跨表按完成时间归并后再分页，`WorkSource` 相应有 `project` / `course_lesson` / `challenge` / `observation` 四种；详情查询允许公开已审核的项目探索记录，按探索会话读取最近 50 条并返回完整计数，无 `exploration_id` 的历史记录只回退当前记录；首页趋势作品仍只读取项目最终作品 |
