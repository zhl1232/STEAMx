...
| courses | `api/courses/` | 技能课程列表/瘦身概览/单课详情；`ldraw-step` 从本地打包 MPD 提取单一步骤及递归依赖，并剔除 LDrawLoader 忽略的 type 2-5 非标准尾字段（16 MiB 源文件上限、同一步骤同飞去重、最多 4 个不同步骤并发转换、浏览器/CDN 短缓存）；课时 `.sb3` 保存与 signed URL；完成课时 +15 XP；`[courseId]/lessons/[lessonId]/works` 读取课时公开作品与提交当前学员作品 |
