...
| `/courses` | `app/courses/page.tsx` | 技能课程列表，**产品主线的唯一入口**（Scratch 编程 + 小班/中班/大班大颗粒积木 + 五子棋博弈论入门等）；页面服务端直接读取已审核课程并输出首屏卡片，避免挂载后再请求 `/api/courses` 的瀑布；登录用户按批量课时进度显示已完成数/总数与未开始、进行中、已完成状态，匿名用户保持无个人进度；页头给一条通往项目挑战的次级出口 |
