...
> 小迪对话前置澄清补充：只有当学生的对象、目标或关键范围不清楚、继续回答可能答非所问时，才用结构化选项确认；清楚的知识问题直接回答，清楚的课程/项目请求直接检索。积木三选一（「通用积木搭建 / 大颗粒积木 / 兼容乐高的积木或零件」）只是示例，不是唯一规则。`lib/ai/tutor/resource-clarification.ts` 负责跨 SSE、活动会话和历史回看的类型与归一化。
...
- 正式品牌名：`STEAMX · 史迪姆`；导航等短位置使用 `STEAMX`，中文品牌名为「史迪姆」。
- 「小迪」仍是 AI 学习导师的对外昵称，不再把「史迪姆」写成小迪的吉祥物全名，避免与平台中文品牌混淆。
- `STEAM` 仍表示科学、技术、工程、艺术、数学这一教育/能力概念，保留在课程、雷达、分类、SEO 关键词和数据库字段中；不要把它机械替换成品牌名。
- 用户可见品牌常量集中在 `lib/brand.ts`；站点 metadata、JSON-LD、PWA 与 llms.txt 通过 `lib/seo/site.ts` 等共享出口使用。域名、路由、环境变量、`x-steam-*` 请求头、包名和历史 LDraw 作者/迁移注释属于内部或历史标识，不因品牌改名而重命名。

### 面向用户的产出词汇：只有三个词

写任何界面文案、通知、AI prompt 或错误提示前先对一遍这张表。之前站内同时用「终稿 / 投稿 / 过程记录 / 观察记录 / 作品 / 项目」六七个词说同一类东西，孩子和家长分不清哪个是哪个，2026-08-14 统一收敛：

| 词 | 指什么 | 覆盖的数据 |
|----|--------|-----------|
| **课程 / 项目** | 你跟着做的那个东西 | `courses` + `course_lessons`、`projects`、`challenges` |
| **作品** | 你做出来的那个东西 | `completed_projects`（`record_kind='final'`）、课时作品、`challenge_submissions`、`observation_events` — 四条路径全部汇入个人主页「我的作品」 |
| **记录** | 过程笔记，不是成品 | `completed_projects`（`record_kind='progress'`，叫「探索记录」）、自然模块的「观察记录」 |

「终稿」「投稿」「过程记录」这三个旧词已从界面移除，代码里的表名列名（`record_kind`、`challenge_submissions`）保持不变，不要为了统一词汇去改数据库。「探索记录」和「观察记录」是有意保留的两个限定说法，它们在各自模块里指不同的过程笔记，都归在「记录」这一类下。
...
| `/` | `app/page.tsx` | 首页 — Hero 使用统一 STEAMX · 史迪姆品牌，并保留 `/explore` 入口，桌面端采用通透呼吸感高度（310px）、多段平滑光影蒙版与微胶囊特性标签；移动端 Hero 下方快捷入口的第一页为「新手推荐 / 挑一节积木课」，其后保持自然观察与排行榜；桌面端在「从这里开始」保留新手推荐（带独立探索行动胶囊）并展示 6 大学科分类入口（带主题微光与 hover 浮起动效）；主体不再重复探索页热门项目，改为统一作品「大家的新作品」与最近公开观察「自然新发现」，桌面双栏、移动端顺序滚动，桌面端在自然板块置顶常驻生态探索横幅，实现与左侧 2×2 作品网格等高平衡；继续保留移动端自然观察/排行榜快捷入口与桌面社区动态（空态引导到 `/explore`）；本周挑战仅在有 featured PBL 时展示并进入 `/pbl/[id]`，不再从首页推广空的创造营/挑战列表 |
| `/explore` | `app/explore/page.tsx` | 探索页 — 专注项目发现，项目列表默认「热门推荐」，并提供「最新上架」「新手推荐」，支持搜索、分类/子分类筛选、精确年龄排序和 `beginner/intermediate/challenge` 三档难度筛选（兼容旧星级/英文参数）；移动端项目网格使用左右对称的页面 gutter；项目卡不再查询或展示已停用的项目评论数，统一展示项目点赞、公开最终作品数和投币三个互补指标，其中作品数通过批量 RPC 获取，避免逐卡查询；探索页及移动端用户主页共用轻量项目卡：保留图片、标题、合并分类、三轴单行摘要和互动数据，隐藏描述与重复主题标签；分页加载时骨架列表与真实项目网格分离，并按整页数量预留，避免 `auto-rows-fr` 重算已有卡片造成滚动跳动；作品预览已移到首页，不在此重复展示；自然观察列表/物种图鉴规范入口为 `/nature/observations`、`/nature/species`，旧 `/explore/observations`、`/explore/species` 及详情地址由 `next.config.mjs` 直接永久重定向并保留查询参数，对应的旧页面壳已删除 |
...
| `/profile` | `app/profile/page.tsx` | 个人主页 — 桌面首屏按「个人 Hero → 本周计划 / 今日行动 → 能力雷达与作品观察摘要」组织，普通桌面主体摘要在宽版卡片内左右并列，大桌面再将经验等级与学习打卡（每日登录流水与学习行为统一计算）放入 400px 右栏（新手引导不再单开面板，已并进本周探索计划，见 `lib/profile/weekly-plan.ts`）；移动端采用现代 Bento 便当盒美学重构为三大高凝聚层级：1.「Hero 个人画像中枢」融合身份称号、徽章阵列、经验条、4 项资产统计与 4 个高频快捷入口（内容/消息/钱包/商店），暗色模式下背景图与遮罩自适应消除泛白漏光，资产条去除了刺眼亮白内高光；2.「探索与成长行动中心」将每日打卡紧凑条与本周探索计划无缝合体（标题栏集成手风琴折叠切换器，任务顺序绝对稳定不跳变）；3.「成长画像与观察成果」将 STEAM 五维能力雷达与自然观察图鉴进度合二为一，突破指南转为标题右上角轻量弹窗，彻底消灭碎片化孤岛卡片堆叠；雷达图、计划卡、打卡与观察卡均支持 `embedded` 容器模式，嵌入大卡片时不重复挂载 `surface-panel` / `profile-mobile-panel`，并在暗色模式下将 `inset 0 1px 0` 硬顶线降为细腻微光（0.06~0.08），消除多重嵌套矩形框与边框断层；移动端标题栏只保留设置，公开主页入口由头像/资料区承担，避免与资料编辑重复；首页作品统计与摘要读取统一作品，内容库区分「我的作品」与用户创建的「发布的项目」；子路由 `library/`、`timeline/`、`likes/`、`followers/`、`following/` |
| `/about` | `app/about/page.tsx` + `app/settings/about/page.tsx` | 公开、可索引的关于与帮助页：平台简介、联系反馈、FAQ、用户协议与隐私政策；反馈仍需登录并发送到站内客服，旧 `/settings/about` 永久重定向到此规范 URL，页脚只链接真实存在的内容锚点 |
| `/settings` | `app/settings/page.tsx` | 用户设置 — 子路由 `profile/`、`appearance/`、`xiaodi/`（小迪语音：自动朗读、移动端长按语音、语音提问播报、提示气泡本设备偏好）、`notifications/`、`privacy/`、`security/`、`safety/`；关于与帮助统一进入公开 `/about`；其余设置页未登录会带 `next` 回到原路径；设置首页菜单使用单层列表与轻分隔，子页由 `app/settings/_components/settings-subpage-shell.tsx` 统一承载，主内容保留单层面板，资料/安全内容使用低对比边界的浅色分组，减少嵌套卡片与边框噪音 |
| `/settings/profile` | `app/settings/profile/page.tsx` + `profile-settings-client.tsx` | 个人资料 — 头像、昵称、简介、性别和出生年月编辑；首屏头像预览优先加载，分组表单使用单层面板与无边框浅色区块，保存后刷新认证资料 |
...
| `/store` | `app/store/page.tsx`、`app/store/orders/` | 实物材料包商城，与 `/shop` 金币虚拟商店完全分离；商品卡片按 `checkout_mode` 区分站内过渡订单与淘宝外部结算，淘宝商品只跳转受限 HTTPS 链接，在淘宝完成支付/物流/售后，不进入本站购物车、地址采集或待支付订单；课程/课时/项目详情页复用上下文材料包卡片 |
| `/admin/store` | `app/admin/store/page.tsx`、`components/admin/store-channel-editor.tsx` | 管理员专用的实物商城运营页：查看淘宝外部结算与备用 1688 目录统计，并为商品配置渠道和淘宝链接；1688 OAuth 仅保留为后续研究状态，不作为当前上线前提 |
...
| `/create` | `app/create/page.tsx` | 项目挑战列表（原创造营，只剩 PBL 一件事），**保留书签路由但不从导航/首页/sitemap 推广**；历史的 `/create?tab=courses` 服务端 301 到 `/courses`，页面不再有 Tab；页头给一条「想先学本领？去技能课程」次级出口；移动端自有页头不显示跳往探索页的搜索入口；`/create` 重定向自 `/community` |
| `/pbl/[id]` | `app/pbl/[id]/page.tsx` | 项目挑战详情 — Hero + 任务说明 + 阶段工作台 + 作品墙；阶段工作台支持保存一句话项目方向并生成每阶段个人化计划提示；移动端任务说明完整展开，底部固定「记录过程 / 提交作品」入口，不在正文重复相关项目；报名走挑战详情自己的 joined 状态，不要求列表页已加载；无历史时返回 `/courses` |
| `/courses` | `app/courses/page.tsx` | 统一技能学习中心，**产品主线的唯一入口**：首屏先突出“推荐起步 · 积木搭建”，同时收录 Scratch 编程、五子棋博弈论等其他系统课程；页面服务端直接读取已审核课程并输出首屏卡片，避免挂载后再请求 `/api/courses` 的瀑布；桌面端使用“技能课程”语义主标题并保留“挑一项技能，做一件作品”的导语；首张课程封面优先加载；登录用户按批量课时进度显示已完成数/总数与未开始、进行中、已完成状态，匿名用户保持无个人进度 |
| `/courses/[courseId]` | `app/courses/[courseId]/page.tsx` | 课程详情与课时列表（左文右图 Hero：五子棋课用纯 SVG 棋盘装饰，其它课走 `image_url` 位图；课时卡带序号棋子 + 课时类型徽章；概览查询只读取课时标题/类型/顺序/时长/分轨摘要 + 一句话简介 + 有无 LDraw 模型，不加载每课完整 content/steps；课时数 ≥ `LESSON_CATALOG_MIN_SIZE`（24）时改用 `CourseLessonCatalog` 目录视图：搜索（标题/拼音首字母/全拼）+ 筛选（有 3D 模型 / 未学 / 已学）+ 拼音首字母分组与 A-Z 快速跳转，少于 24 节仍用平铺列表；Scratch 编辑器及 vendor 不在课程介绍页预加载，只在进入课时后按需启动，课时链接禁用自动 prefetch；登录用户显示课时完成标记和基于 `next_lesson_id` 的开始/继续/回顾 CTA，空课程不显示完成态；整课学完后出现「结课凭证」入口，页头常驻课程分享卡；服务端输出 Course/课时 LearningResource JSON-LD 与面包屑，失效课程 metadata 明确 noindex） |
| `/courses/[courseId]/certificate` | `app/courses/[courseId]/certificate/page.tsx` | 结课凭证与作品册（仅本人可见，未拿到 `user_course_completions` 里程碑则 404）：学员名、课程名、课时数快照、完成日期，下方按时间排本课作品缩略图；可生成 750×1000 分享卡。作品册只收 `status/is_public/moderation_state` 都过审公开的作品（与 `/works/[id]` 同口径，RLS 让本人能读到自己未过审的作品，直接放进作品册会点出 404），其余只在标题右侧报「N 件审核中」 |
| `/courses/.../lessons/[lessonId]/parts` | `app/courses/.../lessons/[lessonId]/parts/page.tsx` | 课时零件清单（有 LDraw 模型的课时才有，服务端渲染）：整课「N 种 · M 块」总表 + 分步新增零件，供家长进搭建区之前先配齐零件；入口在课程目录的课时行与搭建工作区「全部零件」内。没配模型或模型文件缺失才 404；并发超限等临时失败渲染「清单正在生成」并给进课时的出口，因为这些 URL 在 sitemap 里，返 404 会被搜索引擎丢掉 |
| `/courses/.../lessons/[lessonId]` | `app/courses/[courseId]/lessons/[lessonId]/` | 课时学习页（侧栏步骤 + 按 `lesson_type` 切换工作区：Scratch 编辑器 / 大颗粒积木 3D 搭建预览 / 游乐场实训导学；Scratch 手机/桌面能力用 hydration-safe 外部存储快照判断，手机首屏不挂载重型编辑器且保留预览/上传入口；所有 `building_3d` 课时通过统一课程流保留 PPT 第 1 页、隐藏第 2/3 页教学目标与教学流程、隐藏搭建说明 PDF，有真实模型时以全部 3D 步骤替换“作品构建”图片区并保留末尾反思/分享/完成页，无真实模型时保留其余图片页；积木课当前页以一基 `?step=N` 写入路由，离开后返回、刷新及浏览器前进/后退都恢复同一步，并保留 `view` / `ldrawEdit` 等已有查询参数；工作区会预加载下一张课件、下一段视频元数据或下一步 3D 模型数据，LDraw 课从前置课件页开始预热首步与按模型格式拆分的 three.js 运行时，材质和模型并行请求，当前可见步骤完成后才预取下一步以免抢占首屏；移动/平板端提供全屏横向展示，支持 Fullscreen / Screen Orientation API 时自动进入系统全屏并锁定横屏，不支持时回退为页面沉浸模式，3D 横屏采用宽画布 + 紧凑右侧步骤栏；课件图在移动端按原始 16:9 比例决定高度、桌面端限制约 900px 最大宽度，避免媒体区出现大块无效留白；积木课移动端隐藏与工作区翻页重复的侧栏步骤，桌面端继续保留；积木课右侧「本步零件」与可折叠「全部零件」都从模型算：进入搭建区前预取 `/api/courses/ldraw-bom`（取到后翻出/翻回搭建区都不再重复请求），本步显示该步新增块数与零件行（颜色块 + 中文零件名 + ×数量，hover 显示官方英文名），搭建第一步默认展开整套清单（`N 种 · M 块`）方便先配齐零件、后续步骤自动收起且可手动展开；课时 `steps3d` 常比模型多出末尾「完成/对照检查」步（31 个有模型的课时里 9 个如此），这些步与模型内真的没有新增零件的步一样显示 `0 块` 并提示检查已搭部分，**不沿用上一步清单**（3D 画面照旧 clamp 在成品，但零件重复列出会让家长把同一批零件配两遍）；没有 LDraw 模型的课时退回课时 `content.building3d.parts` / `steps3d[].partIds`；3D 用 three.js `LDrawLoader` 经 `/api/courses/ldraw-step` 按 `0 STEP` 顺序读取当前所需几何，复用 Loader 配色/零件缓存并关闭高成本平滑法线预处理，离开 3D 页面或课时时取消请求、释放 geometry/material/texture 与 WebGL context；LDraw 课程模型优先通过 `.agents/skills/image-to-ldraw` 的 `part-metadata.json` 零件定义和 `validate-assembly.mjs` 统一校验支撑、穿模、管道端口连接与方向约束；playground 课时把游乐场游戏包成导学课，右侧「去实战」按钮跳到对应 `/playground/*` 游戏页；playground 课时在移动端用单栏：隐藏左侧 `LessonSidebar` 步骤列表，由 `PlaygroundWorkspace` 承载讲解 + 紧凑进度条 + 上一步/下一步/完成 + 底部「去实战」按钮，桌面端仍保留双栏）；完成课时仅写可信 `user_lesson_progress`，不再发 `+15 XP`，完成整课后按配置一次性生成能力里程碑并刷新页面反馈 |
...
| `/admin/content-classifications` | `app/admin/content-classifications/page.tsx` + `components/admin/content-classification-review.tsx` | 内容分级审核工作台：按内容类型、复核状态和安全提示筛选队列；详情页展示课程课时继承、项目材料/步骤、挑战结构、规则候选与审核历史；支持保存候选、approve/return、并发 revision 校验、幂等提交；审核员禁止自审，管理员自审必须填写原因并写入 `self_review_override` 审计标记。公开三轴标签由课程、项目、挑战卡片/详情页统一渲染，受 `public_v1_enabled` 控制。 |
...
- `app/manifest.ts` / `public/sw.js` / `components/pwa/*` / `lib/pwa/install.ts` / `app/robots.ts` / `app/sitemap.ts` / `app/llms.txt/route.ts` — PWA & SEO；根布局含 `appleWebApp`/`themeColor`，生产环境注册根作用域 Service Worker，并提供中文「安装到桌面」/ iOS / 微信引导；`robots.ts` 为 Baiduspider、GPTBot 及其他常见大模型爬虫明确放行公开页，并与 `User-Agent: *` 共用 `/api/` `/admin/` `/login` 等私有路径 Disallow；Sitemap 使用无请求 Cookie 的公开 Supabase 客户端并按小时 ISR 刷新，动态表统一按 `id` + `range` 每页 500 全量读取，覆盖规范静态页（不含空的 `/create` 列表页）、全部游乐场详情、项目、物种、观察、课程/课时/可用零件清单、公开最终作品、已发布资料卡及 active/ended PBL 挑战；根布局标记 `lang=zh-CN` 与 `applicable-device=pc,mobile`，默认社交图为 `public/assets/seo/steamx-social-card.webp`（1200×630），具体内容图优先；`/llms.txt` 返回 `text/plain` 站点说明
...
> 项目发布口径：普通项目是自主选择的项目模板，用户先开启自己的 Journey，再逐步上传私密过程记录、选择公开审核，最后提交最终作品；PBL 是命题项目，但阶段记录、最终作品、审核和重提仍走同一套 Journey 生命周期。
> PBL 阶段公开入口：`/pbl/[id]` 的 `stage-workspace` 保存阶段产出并关联 Journey 阶段记录，学生可在当前阶段切换「仅自己可见 / 公开审核中 / 已公开」；最终作品继续从 `/pbl/[id]/submit` 提交。
...
| challenges | `api/challenges/` | 挑战列表与评分；作品提交 `[id]/submission`；作品草稿 `[id]/submission/draft` 汇总阶段产出、图片、反馈与 STEAM 收获生成可编辑作品草稿（AI 不可用时回退本地规则草稿）；阶段产出 `[id]/stages`（GET 全部）与 `[id]/stages/[index]`（PUT 落库）；阶段导师反馈 `[id]/stages/[index]/review`（保存当前产出、消耗 AI 配额、生成结构化反馈并写回 `ai_feedback`）；阶段导师工具 `[id]/stages/[index]/coach`（保存当前草稿后生成拆题/提示/总结受控 JSON，仅返回前端展示不写库）；PBL 工作台 `[id]/workspace` 保存个人项目方向并返回受控个人化计划 |
...
| completions | `api/completions/` | 完成记录、作品评论、点赞、审核；`[id]/promote` 通过作者专用事务 RPC 将已审核项目探索记录设为完成作品，拒绝旧的最终作品可重提，并将并发唯一冲突返回 409；发表评论/回复需要社区互动确认和双向屏蔽检查，评论可按 `completion_comment` 举报 |
| discussions | `api/discussions/` | 社区讨论 CRUD、点赞 |
| follows | `api/follows/` | 关注/取关、关注状态查询 |
| blocks | `api/blocks/` | 双向屏蔽关系查询、创建、解除；屏蔽后禁止私信/关注/点赞/收藏/评论/打赏等社区互动，凡是会给对方生成通知的入口都要走 `assertUsersNotBlocked` |
| geo | `api/geo/` | 高德地点搜索、逆地理编码与 GPS 坐标转换；地点名称只来自照片 EXIF GPS 的反查或用户手动搜索/地图选点，缺少高德 Web 服务 Key 时 GPS 转换回退本地 WGS-84→GCJ-02，搜索/名称反查仍需配置 Key |
...
| store | `api/store/`、`api/admin/store/products/[id]/` | 实物商城商品目录、淘宝外部结算链接、用户订单/订单详情、支付通道占位与签名 webhook；公开目录只返回安全展示字段，服务端拒绝用站内报价接口购买外部商品；`alibaba/oauth/*` 仅 admin 发起 OAuth、服务端加密保存 1688 token，`api/internal/store/sync` 仍是后续履约入口但当前不启用自动下单 |
...
| profile | `api/profile/` | 个人资料摘要、新手引导、学习打卡、本周探索计划（每日打卡汇总统一读取登录流水、项目/观察/挑战行为）；`equipped-honor` 支持一键佩戴/卸下称号（`equipped_title`）与配置主页徽章佩戴列表（`featured_badge_ids`）：`NULL` 表示未手动配置并使用默认精选，空数组表示明确不佩戴，非空数组表示按顺序手动佩戴且最多 5 枚；公开主页和个人主页首屏都露出最多 5 枚，超出部分以 `+N` 查看其余已解锁徽章；满 5 枚后佩戴新徽章必须在替换框中选择被替换的位置，替换后保持其余顺序，移出最后一枚会保存空数组；空数组主页只保留“徽章图鉴”入口，公开主页只读；图鉴详情会列出实际佩戴成员和默认/手动来源；`works` 返回当前用户的四类作品（项目 / 课时 / 挑战 / 自然观察）；`growth-tasks/sync` 与 `weekly-plan` 都会读取 `profiles.bio` 并调用 `get_user_stats_summary` 计算成长任务进度 |
...
| tips | `api/tips/` | 打赏；打赏会给对方推一条带昵称头像的通知，所以和点赞/评论一样先做双向屏蔽检查，屏蔽关系下直接 403 且不扣币；`tips/my` 匿名直接返回 `myTipped: 0`，不走需登录的限流桶（否则 F12 会看到 429） |
...
| users | `api/users/` | 用户公开信息查询；`[id]/works` 返回指定用户已公开的统一作品，非 UUID 的 id 直接 404（否则 Postgres 22P02 会以 500 透出） |
...
互动资格边界：课程完成、Scratch/积木项目保存、PBL 阶段与工作区保存、项目探索和私信发送登录即可；项目/课程/挑战作品提交、自然观察提交、评论/鉴定/评分需要社区互动确认，内部由 `age_confirmed_at` 记录；私信另受接收方隐私设置、双向屏蔽、频率限制、内容审核和 `interaction_restricted` 控制；项目/作品/自然观察点赞、收藏、关注和打赏属于 `engage`，登录即可，但都另受双向屏蔽约束。项目上下文的收藏写入统一经 `POST /api/projects/[id]/collection`，不再由浏览器直接写 `collections` 表。

---

## 3. 组件 (`components/`)

### 3.1 基础 UI (`components/ui/`) — 40 个组件
基于 shadcn/ui + Radix UI 的基础组件库：
`alert` · `avatar` · `avatar-with-frame` · `badge` · `button` · `card` · `checkbox` · `countdown-timer` · `dialog` · `difficulty-stars` · `dropdown-menu` · `filter-chip` · `image-upload` · `input` · `label` · `leaderboard-skeleton` · `loading-skeleton` · `mobile-page-header` · `optimized-image` · `page-status` · `progress` · `radio-group` · `report-dialog` · `role-badge` · `scroll-area` · `search-highlight` · `select` · `separator` · `sheet` · `skeleton` · `slider` · `surface` · `table` · `tabs` · `textarea` · `toast` · `toaster` · `tone-badge` · `user-avatar`
- `components/ui/user-avatar.tsx` — 公共用户头像统一入口：复用 `AvatarWithFrame`，按用户 ID 选择稳定默认头像，默认链接 `/users/[id]`；头像已处于其它链接或按钮时显式传 `href={null}`，尺寸仅由场景 class 控制。
...
- `conditional-app-shell.tsx` — 根据路由条件渲染 Header/BottomNav/Sidebar；公开页挂载服务端 `SiteFooter`，登录、分享、后台、课时与游乐场对局等沉浸页隐藏；`LoginPromptProvider` / `GamificationProvider` / `NotificationProvider` 必须无条件渲染，包装层数量不能随登录态变化，否则鉴权返回时 React 会把整棵页面子树卸载重挂（匿名态下这两个 Provider 本身就空转）
- `site-footer.tsx` — 全站页脚（服务端组件）：桌面保留栏目链接，全端展示 ICP 备案号 `京ICP备2025129751号-2` 并链到工信部，保证出现在 SSR HTML
- `bottom-nav.tsx` — 移动端底部导航（首页 / 探索 / **课程** / 自然 / 我的）
- `main-nav.tsx` — 桌面端顶部导航（首页 / 探索项目 / **技能课程** / 自然观察 / 益智游乐场）
- 两处导航都把「创造营」换成了主线课程入口，项目挑战降为次级：`/create` 与 `/pbl/*` 归到「课程」这一项的 active 判定里，不再单独占一个顶层槽位。要加新频道前先想清楚它是不是又一个平级「从这里开始」——改版前正是七个平级入口指向五个频道，谁都不是主线
...
- `interaction-confirmation-dialog.tsx` — 发布作品、评论或私信前的社区互动确认弹窗；确认成功后由 `login-prompt-context` 重试原操作
...
| 子目录 | 文件数 | 职责 |
|--------|--------|------|
| `bird-observation/` | 23 | 观察提交表单、照片上传、Leaflet 地图选点/热点预览、观察卡片、物种热点面板、物种统计面板（无观察记录时隐藏）、评论区；地图引擎从本站静态资源加载，底图继续使用国内高德瓦片；`observation-location-picker` 将地点输入、搜索结果、地图和当前位置放在同一视图；提交一次可多选最多 5 张照片，每张单独成为一条观察，物种/性别/阶段/地点/拍摄时间跟这张图走；EXIF 只预填该图，禁止用首张 GPS 覆盖其他图，手动改名会清除旧坐标；无 GPS 的图需搜索或地图选点，可选「用到其余未定位照片」；AI 识别只给物种建议，低质量/未识别/处理中不阻塞提交，仅安全审核失败阻塞；新记录始终公开并使用准确位置；主流程无描述；观察详情轮播复用共享 `SwipeablePhotoViewer` / `ImageLightbox`，当前缩略图使用 eager 加载、其他缩略图保持 lazy，保证切换后的选中缩略图立即可见；观察详情 AI 鉴定头像使用 `public/xiaodi-ai/idle-0.webp` 小迪静帧 |
| `challenge/` | 5 | 挑战提交表单（新建时按阶段产出汇总预填，并可一键整理成可编辑作品草稿：标题、作品说明/反思、阶段图片与 STEAM 收获）、PBL 信息 `pbl-info`（「相关资料」按 参考项目/前置技能/资料卡 三分类分组渲染，带描述行）、评分星级、阶段工作台 `stage-workspace`（逐步解锁引导：未解锁阶段不渲染，仅显示"还有 N 步"折叠提示；支持保存个人项目方向并显示每阶段个人化计划；阶段产出防抖自动保存，唯一主按钮「完成这步」+完成清单(成功标准)+导师工具「帮我拆题 / 给我提示 / 整理这步」返回受控参考卡；「请导师看看这步」生成并持久化 做得好/还缺/下一步 反馈卡；注册小迪 `pbl.focus_current_stage` 工具 handler，在卡住/下一步/反馈意图下展开并高亮当前阶段）、提交作品画廊 |
| `courses/` | 19 | 技能课程列表、课时侧栏与工作区路由；`course-board` 由 `/courses` 服务端直接传入课程（原先给创造营页签用的 `course-board-loader` 已随 Tab 一起删除）；`course-lesson-catalog` 承担 ≥24 节的大课目录（搜索/筛选/拼音分组/A-Z 跳转，逻辑在 `lib/courses/lesson-catalog*.ts`），课时行直达零件清单页；`course-share-button` / `certificate-share-button` 按需加载分享卡；`lesson-completion-toast` 在整课学完时把结课凭证入口带进 toast；Scratch 持久 Host 复用单一 iframe 并支持作品发布，Scratch 工作区支持“自检这步”，按当前选中对象的 VM 积木、参数与连接关系给出已完成/未找到/需核对，并可把下一处缺项交给现有积木高亮；积木定位已覆盖四则运算与 `x/y 坐标`、方向、大小、音量 reporter；大颗粒积木工作区用 three.js/LDraw 渲染课件与分步模型，游乐场工作区承载导学和完成课时；三个 workspace 的完成反馈不再显示 `+15 XP`，而是显示进度保存或整课 STEAM 更新；`content.workSubmission.enabled` 为 true 时，`lesson-works-gallery` 直接读取当前课时公开作品，`lesson-work-upload` 复用完成作品弹窗并提交到课程课时 works API，无需项目 Provider 或背书项目；课程作品上传成功后进入统一作品详情并直接打开分享卡片 |
| `works/` | 4 | 统一作品 UI：`work-card-grid` 为探索页、课程与个人主页提供响应式作品卡片，封面不再叠「课程/项目」标签，类型徽章与标题同一行（课时 / 项目 / 挑战 / 自然观察四种来源各有配色与落点），下一行放课程名或作者与点赞、留言数；`work-detail` 展示媒体、来源、作者、评论和审核状态，项目探索时间线最多展示最近 50 条并在截断时标出总数，点赞/投币与作者信息同行，作品留言的回复/举报控件保留至少 44px 触控命中区，投过币的作品会在投币按钮上显示琥珀色已支持状态，空创作说明不占位，并仅向作品作者开放分享交互；`work-image-gallery` 提供预览、缩略图跳转和全屏看图，时间线内按竖图比例居中展示并裁切在卡片内，避免横框留白溢出；`share-work-dialog` 内的 `ShareCardDialog` 统一为作品、项目、**课程和结课凭证**四种 `kind` 生成固定画布高清卡片与高纠错二维码，并提供系统分享、保存图片和复制链接 |
| `community/` | 1 | 讨论列表（含搜索、排序、分页） |
| `gamification/` | 15 | 徽章图标/画廊、等级进度、排行榜、成就 Toast、每日登录同步（登录用户首页也挂载，临时失败自动重试）、观察游戏化同步；主页佩戴列表手动最多 5 枚、默认精选最多 5 枚，公开主页和个人主页首屏都露出 5 枚，超出部分以 `+N` 查看其余已解锁徽章；NULL 未手动配置时使用默认精选，空数组明确表示不佩戴；图鉴按系列只露最高档，点开 `badge-detail-dialog` 看档位清单与进度，详情中的主页徽章区列出实际佩戴徽章名称和`默认佩戴/手动佩戴`来源；已佩戴徽章显示独立的“移出”按钮，满 5 枚时新徽章必须先选择替换位置并保留其他顺序；详情顶部明确区分当前档位的「已解锁/待解锁」与下一档目标，使用“当前值 / 目标值”进度条和“距离目标还差”文案，避免把下一档进度误读成当前徽章状态；昆虫钻石未解锁不出现；自然观察徽章 PNG 使用透明背景，资源 URL 带版本参数避免线上图片优化缓存继续返回旧黑底图；徽章解锁先写入轻量 `badge-unlock-store`，`badge-unlock-overlay-mount` 只有队列非空时才加载 confetti/Framer Motion 庆祝层；单枚全屏庆祝可点跳过/超时关闭，同批 ≥2 枚改为汇总网格一次看全并链到 `/profile` 图鉴 |
| `moderator/` | 2 | 审核员申请表单 |
| `tutor/` | 18 | 全局 AI 导师「小迪」（隶属 STEAMX · 史迪姆平台）：`tutor-context` Provider（含场景 override、Scratch 编辑器上下文、scene capability、待发送消息队列与白名单 tool handler 注册/分发），`tool-handler-registry` 负责把后端 tool 名映射到当前页面提供的“聚焦课程步骤 / 聚焦 PBL 阶段 / 扫雷公开棋盘提示”等前端能力，并反推出当前 scene capability 供请求一并上送，避免各页面自己逐个绑定工具名；`scene-capabilities` 同时管理页面动作能力和回复增强能力，`speciesAudio` 仅在物种档案/自然观察记录有鸟类音频时由服务端场景授权，课程场景不自动补鸟鸣；`global-tutor-mount` 按路由感知场景（含课时页 `lessonId`）并让 React Query 预取延后到小迪模块加载后、`global-tutor-fab` 已从巨石组件拆为编排层（约 700 行）+ 专职模块：`tutor-stream-protocol`（SSE `data:` 行解析，跨 chunk/UTF-8 安全）、`use-tutor-chat-stream`（消息状态、流式请求、AbortController、tool call 分发、session 缓存）、`tutor-pcm-player`（24kHz PCM Web Audio 排队播放）、`use-tutor-voice`（录音/转写/流式 TTS/自动朗读/移动端长按桥接）、`use-tutor-history`（历史列表/详情/删除/新话题）、`use-tutor-fab-drag`（悬浮球拖拽与长按互斥）、`tutor-message-list` / `tutor-composer` / `tutor-voice-feedback`（消息区、输入区、语音反馈 UI）；发送用 AbortController 持有在途 SSE（场景切换/卸载时中止旧流，防止旧话题内容和 busy 态串进新场景；主动中止静默清掉流式占位气泡，服务端仍会完成落库），使用 `<XiaoDi>` AI 8 帧候选作悬浮球，面板头部同一只小迪随 `idle/listening/thinking/speaking/working/success/error` 状态切换（录音时 listening、转写和请求时 thinking、流式输出和语音朗读时 speaking、tool call/图片上传时 working、工具或回复成功后短暂 success、配额/请求/工具/上传失败时短暂 error，success/error 经 `onCycleEnd` 回落），并保留流式对话（输入区为上文本下工具栏：左传图 `+`、右语音/发送，多行 textarea 默认约 2 行并随内容增高至上限；聊天框可直传图片，审核拒绝时展示后端安全原因；面板内语音按钮经 `tutor-voice` 采集 16k PCM 后调用后端转写并回填输入框，录音/识别/准备朗读/朗读期间显示声波或加载状态反馈与录音计时；移动端关闭态可长按小迪直接录音，松手转写后自动发送，并按本设备偏好播报这次回复，关闭态也显示语音状态浮层；小迪头上会按节流偶尔显示“长按说话”提示；小迪回复可逐条手动朗读，⋯ 菜单含默认开启的“自动朗读新回复”总开关，完整语音偏好在 `/settings/xiaodi` 调整，TTS 默认音色为更中性的 `Ethan` 且可用环境变量覆盖，自动朗读在 chat SSE 里边出字边播 PCM，点喇叭同样走流式合成，HTTP 整段音频仅作回退；场景照片一键发图、Scratch 课时页紧凑位；打开时优先消费预取缓存，⋯菜单含「开启新对话」与「历史对话」，归档线程列表+只读回看视图，并可删除历史对话（归属校验后级联清理消息）；消费 SSE `tool_call` 事件并交给当前场景 handler，支持 PBL 阶段聚焦、课时步骤聚焦与扫雷非透视提示；发送 Scratch 课时消息时附带当前选中角色/对象，避免默认说“小猫”）；图鉴首页移动端隐藏全局悬浮入口，避免遮挡 tile；`tutor-session` 会话 query key/fetch helper、`tutor-message-content` 回复轻量 Markdown 渲染 + Scratch 分类图例/积木形状富文本 + `[project:ID|标题]` 项目 chip + 经 `speciesAudio` 授权的 `[audio:slug|物种名]` 内联鸟鸣播放器、`xiaodi.tsx`+`xiaodi.module.css` 小迪吉祥物动画组件 `<XiaoDi state size onCycleEnd variant />`（默认 7 状态 idle/listening/thinking/speaking/success/error/working；`variant="default"` 读取 `public/xiaodi/sprite.webp`，默认 `variant="ai-draft"` 读取 `public/xiaodi-ai/sprite.webp`，运行时每个变体只加载一张 sprite 并用坐标切帧 + 状态化 CSS 补间：呼吸/前倾/摇摆/点头/弹跳发光/歪头/顿挫；状态切换 160ms 淡入淡出、关闭态静态入口只加载轻量 `idle.webp`，完整 sprite 在浏览器空闲或首次交互后加载，切换变体前等待目标 sprite decode 且保留上一姿势，避免加载期透明闪帧；AI 候选 idle/listening/thinking/speaking/error/success/working 分别约 3.6s/3.2s/2.8s/2.8s/1.8s/1.5s/4.2s 一轮；`prefers-reduced-motion` 降级静帧、`onCycleEnd` 支持 success/error 播一轮切回 idle） |
...
| `project/` | 9 | 完成项目弹窗（作品照片支持一次多选、继续追加并按选择顺序提交，最多 9 张；最终作品提交后把作品 ID 传给详情页并自动打开分享卡片）、项目详情操作栏（项目分享接入统一 `ShareCardDialog`）、打赏弹窗、续做卡片、`project-hero-gallery`（封面+步骤图左右滑动/全屏预览） |
| `journey-record-timeline.tsx` | — | 普通项目当前 Journey 的本人过程时间线；展示私密、公开审核中、已公开和拒绝意见，并支持单条切换可见性 |
| `social/` | 3 | 关注按钮、用户屏蔽按钮 |
| `shared/` | 4 | 通用评论卡片、底部回复框、可左右滑动的 `SwipeablePhotoViewer`、全屏 `ImageLightbox`（左右滑动切图、双指/滚轮缩放、双击放大、下滑关闭；作品详情、观察详情与项目详情 Hero 共用） |
...
移动端个人主页、资料头部、作品库、点赞列表、时间线、用户列表、项目列表；移动端项目列表复用探索页 `ProjectCard` 轻量竖版卡，桌面内容库和公开主页桌面端保留完整默认卡片
...
- `nature-observation-observed-species.ts` — 已观察物种统计：请求级调用无 user id 的 `get_my_observed_species_ids()`，审核通过记录上优先取社群共识物种，否则取 AI 置信度 ≥ 0.8 的鉴定结果；无 Supabase auth cookie 的匿名/爬虫请求直接返回空进度，避免公开图鉴多一次认证往返；保留事件批量读模型供历史进度/生命周期数据使用
- `observation-gamification.ts` — 观察游戏化：审核通过（`status` + `moderation_state`）后发 XP，并按条数/种数/鸟三堆/昆虫九宫格补发徽章；只插入缺失、打回时收回不合格记录，保留已有 `unlocked_at`
...
- `lib/geo/wgs84-gcj02.ts` — 高德 Key 缺失时为照片 EXIF GPS 做本地 WGS-84→GCJ-02 转换，地图名称仍由逆地理编码服务或用户手动选择提供
...
- `interaction-access.ts` — 统一互动资格判定：匿名/已注册/已确认/restricted；保存进度和私信发送登录即可，作品提交、评论等公开内容写入需要社区互动确认
- `challenge-submission-validation.ts` — 挑战作品标题/说明/图片说明敏感词校验，证明图片/视频必须来自当前账号上传
...
- `badges.ts` — 全部徽章定义（独立/阶梯/系列）；阶梯系列用 `tierNames` 独立成就名，档位可用 `BADGE_TIER_LABELS` 作说明文本，昆虫观察（初识虫趣/寻虫常客/寻虫能手/虫林专家/昆虫传奇）额外有 `diamond`；图鉴与公开主页经 `getSeriesDisplayBadge()` 每系列只露最高档，未解锁钻石不出现；公开主页通过 `getBadgeDisplayDefinitions()` 去掉仅用于判定的 `condition`，保证 RSC 到客户端的徽章数据可序列化；STEAM / 项目完成 / 初级探索者按项目+课时/观察计数；社区 `social` 只数留言+回复；自然观察保留条数 `bird_observer` 与种数 `species_collector`，并增加常见鸟 / 进阶鸟 / 稀有鸟与昆虫九宫格观察阶梯；游乐场 `playground_explorer` 门槛 3/8/13/18；内测徽章 `beta_tester` 由 `GRANT_BETA_TESTER_BADGE` 控制
- `species-difficulty.ts` — 站内 133 种鸟的常见/进阶/稀有对照，以及昆虫八套九宫格与专属锹甲/大蚕蛾/大步甲/北京神物挑战名录
...
- `experience-rules.ts` — XP 经验规则与等级表；每日登录同步由 `DailyCheckInSync` 调用 `daily_check_in`，成功后触发连续打卡徽章检查。**只列真的会发的奖励**：讨论区下线后 `create_discussion` / `reply_or_comment` 已删除，课程作品用 `publish_course_work`；等级权益说明（`level-guide-dialog.tsx`）同理，Lv2 发图、Lv30 八折、Lv50 社区自治这些没实现的承诺已经拿掉，加新权益前先确认它真的存在
...
- `project-citability.ts` — 项目页面向豆包、文心、通义的引用摘要：按项目类型组合年龄、reflection 或描述与关键词；项目 JSON-LD 同时输出材料/步骤 HowTo 和 reflection 结论 FAQ，页面移动端与桌面端都显示结论区块
...
- `weekly-plan.ts` — 个人主页「本周探索计划」纯规则生成：按上海时区每周一 00:00 重新计算 3-5 步学习路径；已完成区只统计当前周有行动意义的里程碑（项目发布/完成、挑战提交、自然观察，最多 3 条），每日签到、经验流水和徽章解锁继续显示在每日打卡/成长足迹中但不占计划名额；进行中的 PBL 阶段、课程和探索中项目跨周延续；**新手引导已并进这里，个人主页只保留这一个待办概念**（原先桌面端同屏挂着 `PrimaryPlanCard` / `ProfileStarterHub` / `GrowthTasksPanel` 三块说同一件事的面板，后两块已删；未毕业时在计划卡头部显示一行「新手引导 N/5」保住毕业进度感）。起步周计划的三步直接由新手引导任务定义生成，让「这周要做的事」和「能领的奖励」是同一批对象；同一摘要供小迪 profile 场景解读
- `growth-tasks.ts` — 新手引导系统（内部模块名仍为 growth-tasks）。主线三步对齐积木课：开始第一节课 → 学完第一节课 → 发布第一件作品，配合自我介绍与首次互动共 5 项；进度靠 `get_user_stats_summary` 的 `lessonsStarted` / `lessonsCompleted` / `worksPublished`（`20260814150000_stats_lesson_and_work_counts.sql`）
- `steam-radar.ts` — STEAM 能力雷达图数据；`calculate_steam_radar` 吃项目终稿、挑战完成、整门课结课，以及 `status` 与 `moderation_state` 都过审的观察（只进 S，权重 15、按 3 星衰减；私密也算本人）
- `study-checkin.ts` — 每日打卡展示逻辑；后端 `get_user_study_checkin_summary` 从 `xp_logs` 的每日登录流水恢复历史登录日，并与已通过项目最终作品、已通过观察及待审/已通过挑战作品合并计算连续天数
- `settings.ts` — 设置项读写
...
| 模块 | 文件 | 职责 |
|------|------|------|
| `lib/content-classification/` | `types.ts`, `constants.ts`, `labels.ts`, `mapping.ts`, `rules.ts`, `validation.ts`, `queries.ts`, `json-ld.ts` | 课程、项目、挑战共用内容分级领域层：推荐起始年龄、三档难度、成人支持度、国内学段/K–12 交换映射、候选规则、公开 DTO、年龄排序和失效字段校验；`difficulty_stars` 只在 mapper 中派生公开难度，公开开关开启后 reviewed 内容输出三轴，未复核内容统一返回 `classification: null` |
...
| `lib/pbl/` | `challenge-workspace.ts`, `challenge-stage-review.ts`, `challenge-stage-progress.ts`, `stage-coach-actions.ts`, `challenge-submission-draft.ts` | PBL 工作台个人项目方向、个人化计划 JSON 类型、确定性计划生成与数据库行映射；阶段导师反馈上下文/产出摘要构建；阶段产出快照比较与反馈失效判断；导师工具动作（拆题/提示/总结）受控结果归一化；作品草稿规则汇总、STEAM 收获提取与 AI 草稿归一化 |
...
| `lib/store/` | `service.ts`, `external-channel.ts`, `http.ts`, `address-crypto.ts`, `secret-crypto.ts`, `alibaba-1688.ts`, `alibaba-order.ts` | 实物商城目录/报价/订单服务（MOQ、库存、单供应商代发和幂等）；外部渠道 URL 白名单、上下文关联键与公开目录映射；AES-256-GCM 地址与 1688 token 密文；1688 OAuth、AOP HMAC-SHA1、HTTPS/超时/有限重试客户端；自动下单适配器仅作为后续方案保留，当前不启用未经确认的 `fastCreateOrder`；明文地址、token 和供货映射不进入客户端 |
| `components/store/` | `contextual-store-products.tsx` | 课程、课时零件清单和项目详情中的上下文材料包卡片；仅展示已配置且通过淘宝 HTTPS 白名单的外部商品 |
...
| `lib/utils/` | 文件校验、HTTP 工具、上传、手机号、拼音、自然导航、主题分类；`safe-internal-href.ts` 校验站内 `from`/`next`（允许 `/`，拒绝 `//`、反斜杠与 `..`）；图片上传会按 bucket 在客户端预压缩，其中作品图片 `project-completions` 最长边限制为 1600px、目标不超过 0.9MB，降低多图作品加载量；`bounded-ttl-map.ts` 提供无定时器、带 TTL 与容量上限的进程内 LRU，避免用户/IP 维度缓存随长期流量无限增长；`ldraw-mpd.ts` 负责打包 MPD 的拆分/按 `0 STEP` 分步/内联零件注入，`ldraw-bom.ts` 复用同一套分步逻辑统计零件清单（每步新增块数 + 全模型总数，按零件×颜色分组，跳过 LDraw 图元/子件，`~Moved to` 别名跟一跳，零件名与 LDConfig 色名转中文，认不出时保留官方英文） |
...
| `lib/auth/` | `server.ts`, `login-redirect.ts` | 服务端认证辅助；`requirePageUser()` 默认按 proxy 转发的当前路径生成 `/login?next=...`，受保护分区的 layout 不要再传写死的 `next`（会把 `/messages/[userId]`、`/profile/timeline`、`/share?edit=` 这类深链接压成分区首页）；旧 `/settings/about` 仍被识别为公开路径并永久重定向到 `/about` |
| `lib/testing/` | `playwright-smoke.ts` | E2E 测试辅助 |
| `lib/membership.ts` | `membership.ts` | 会员档位/周期、有效性判断与 AI 代币常量（免费 5 次/天、会员月发 1500 代币、图文扣费 1/2）；`membership_*` 四字段由 `protect_profiles_sensitive_fields` 锁定，后台开通走 `admin_set_membership` RPC |
| `lib/courses/` | `types.ts`, `lesson-types.ts`, `building-lesson-flow.ts`, `device.ts`, `scratch-messages.ts`, `scratch-validate.ts`, `scratch-hints.ts`, `scratch-step-check.ts`, `progress.ts`, `config.ts`, `reconcile.ts`, `lesson-catalog.ts`, `lesson-catalog-builder.ts`, `ldraw-bom-source.ts`, `certificate.ts` | 技能课程课时类型（scratch / building_3d / playground；未知 slug 回退 unsupported）、课时步骤可选结构化图解类型（目前 `gomoku_board` 支持黑白子、候选点、辅助线、获胜线），`building-lesson-flow.ts` 将积木课 PPT、动画和真实 3D 步骤归一成侧栏与工作区共用的连续课程流，3D 步骤 `cameraHint` 支持 front/back/side/top/isometric 视角、根级 `workSubmission.enabled` 控制该课是否支持直接发布作品（`learningGoals` / `teacherGuide` 已于 2026-08-14 删除，线上 316 节课这两个字段全为空，历史文案见 `20260627172000` 等迁移）；`lesson-catalog.ts` 是纯客户端目录逻辑（搜索归一、拼音首字母分组、有模型/未学/已学筛选、`LESSON_CATALOG_MIN_SIZE=24` 阈值），`lesson-catalog-builder.ts` 在服务端用 `pinyin-pro` 预生成首字母与可搜索文本，避免把拼音库打进客户端包；`ldraw-bom-source.ts` 收口零件清单的本地 MPD 读取、10 分钟 mtime 缓存、并发上限与 `LdrawBomBusyError`，同时供 `/api/courses/ldraw-bom` 和零件清单页使用，页面走 `lookupLdrawBom()` 拿 `ok / missing / unavailable` 三态（没配模型或文件缺失是 `missing`，并发超限等临时失败是 `unavailable`），别把两者混成一个 `null`；`certificate.ts` 读取结课凭证（里程碑 + 课程 + 学员 + 本课已过审公开作品，另返回 `pendingWorkCount`），未拿到里程碑或课程未审核一律返回 `null`；`building_3d` 内容优先用 `ldrawModelUrl` 指向自托管 LDraw `.mpd` 模型，模型内 `0 STEP` 驱动分步显隐，可选 PPT 逐页图、动画、历史搭建说明和成品图；`brickInstances` 仅作为历史/开发兜底，不用于新增大颗粒课程；`progress.ts` 负责空课/匿名/排序稳定的课程进度纯计算、完成反馈和 DTO 映射，`config.ts` 统一 Admin 与运行时 STEAM 权重校验，`reconcile.ts` 触发服务端补偿；其余文件负责设备能力判断、Scratch iframe postMessage 协议、`.sb3` 积木校验、小迪 Scratch 积木提示与当前步骤自检（opcode、字段/输入值和连接关系） |
| `lib/works/` | `types.ts`, `capability.ts`, `data.ts`, `submission.ts` | 统一作品领域层：判断课时是否支持作品、读取/映射作品、提交课程作品、按来源补齐课程或项目信息；`getUserWorks()` 把**四条产出路径**汇总成一份「我的作品」——项目最终作品、课程课时作品、PBL 挑战作品（`challenge_submissions`）、自然观察（`observation_events`），跨表按完成时间归并后再分页，`WorkSource` 相应有 `project` / `course_lesson` / `challenge` / `observation` 四种；详情查询优先按 `journey_record_id` 读取公开已审核的 Journey 过程记录（保持多次尝试边界），仅对迁移前没有 Journey 关联的历史记录回退 `exploration_id`；首页趋势作品仍只读取项目最终作品 |
...
| `lib/api/weekly-plan-data.ts` | `weekly-plan-data.ts` | 本周探索计划服务端数据聚合：并行读取个人作品/雷达/新手引导/自然观察、本周项目/挑战/观察里程碑、进行中 PBL 阶段与在学课程，返回共享 `WeeklyPlan`；签到经验和徽章由独立打卡/成长足迹数据流处理，不参与计划名额 |
...
| `docs/STORE_CHANNEL_FULFILLMENT_DECISION.md` | — | 实物商城渠道与履约决策：当前采用 STEAMX 商品卡片 + 淘宝外部结算（淘宝负责支付、物流、售后），保留少量人工采购兜底；达到主体、订单量和平台资质门槛后再评估开放平台/合规 ERP；不申请 1688 铺货分销 ISV，不直接 iframe 嵌入外部收银台 |
...
- `lib/home-featured-slides.ts` — 首页轮播配置（已去掉创造营 slide；当前未接入首页 UI）
- `lib/product/mainline.ts` — **主线常量，新增入口/CTA 前先看这里**。全站只认一条「第一次成功」路径：挑一节积木课 → 跟 3D 分步搭出来 → 拍照上传 → 拿到反馈。首页桌面/移动主 CTA、注册成功后的落地页、起步周计划三处都从 `MAINLINE_ENTRY_HREF`（`/courses`）与 `MAINLINE_PRIMARY_CTA_LABEL` 取值，别再各自硬编码，否则又会退回改版前「七个平级从这里开始、指向五个频道」的状态。注册落地要认 `sms/verify` 回传的 `isNewUser`，不能拿表单上的登录/注册切换来判断——真实注册走的就是默认那个手机号验证码表单，`emailMode` 一直是 `sign_in`。北极星指标是注册后 7 天内产出第一件作品的人数
- `lib/messages/message-time.ts` — 私信连续消息时间分组与本地时间分隔格式化
- `lib/messages/stranger-gate.ts` — 私信递进门槛（纯函数，`app/api/messages/send/route.ts` 消费）。关系分三档，看的是「对方回复了没有」而不是「发了几条」：**陌生人（非互关）在对方回复前只能发 1 条**、**互相关注 3 条**、**对方已回复后正常聊**（仍受 20 条/60 秒限流）。`followers_only` 隐私档是**互相关注**语义，单向关注不解锁（否则陌生人点一下关注就绕过去了），但**对方已回复过的会话直接放行**，回复本身就是最强的同意信号，不然接收方切档位会把正在进行的对话单方面掐断；「不能带外链/邮箱/手机号」的限制覆盖整个陌生人阶段而不只是第一条。额度是常量 `STRANGER_MESSAGE_LIMIT` / `MUTUAL_FOLLOW_MESSAGE_LIMIT`，错误文案要说清怎么解锁。路由侧用「对方发过消息没有」的存在性查询 + 自己发出条数的 count 查询来判定，不要退回按会话前 N 条推断，历史长会话会被误判成陌生人；条数检查是先读后写，并发下有可能多放一条，属于已知取舍
...
- 本次安全加固迁移：`20260803120000_harden_interaction_access_and_xp.sql` 撤销旧 XP RPC 的公开执行权、禁止客户端写 `xp_logs`，通过 `current_user_can_interact()` 与数据库触发器封住项目/评论/作品/观察/消息/互动/进度的直接写入绕过，并在 `award_xp_once()` 内恢复项目评论每日 50 XP 上限；迁移末尾通知 PostgREST reload，应用使用 `pnpm db:push`
- 本次新增社区安全治理迁移：`20260804120000_community_safety_governance.sql` 增加 `user_blocks`、`moderation_cases`、`safety_actions`、`safety_appeals` 与账号安全投影；项目、讨论、评论、完成作品、自然观察、挑战作品和私信增加 `moderation_state`，公开读取仅允许 `approved`，本人/审核员仍可查看审核态；举报可自动隐藏高风险内容，安全处罚支持互动限制、停用、封禁与有效期内申诉，证据按保留期清理；应用使用 `pnpm db:push`，不要使用 `supabase db push`
...
- 本次项目卡作品数迁移：`20260810190000_project_completion_counts_batch.sql` 增加公开最终作品的项目维度部分索引和 `get_project_completion_counts_batch()` 批量 RPC；函数使用调用者权限并显式过滤 `is_public/status/moderation_state/record_kind`，应用使用 `pnpm db:push`
- 本次作品审核奖励修复：`20260810200000_fix_completion_reward_uuid.sql` 让审批与奖励修复 RPC 的 XP 流水变量跟随 `xp_logs.id` 的 UUID 类型，恢复最终作品自动/人工审核与幂等 `+20 XP`，并移除相关内部 RPC 遗留的匿名执行授权；应用使用 `pnpm db:push`
- 本次探索记录转最终作品迁移：`20260810210000_promote_progress_completion_to_final.sql` 增加仅本人可调用的 `promote_progress_completion_to_final()` 事务 RPC；只允许把已公开审核的项目探索记录设为最终作品，并原子处理最终作品唯一性、幂等奖励与探索会话完成状态；应用使用 `pnpm db:push`
- 本次内容分诊硬删除：`20260813090000_hard_delete_triaged_projects.sql` 按站长 2026-08-13 审定的 105 个项目 ID 永久删除（不是下架），并先清子表/关联行；权威名单与保留 ID 见 `scripts/lib/content-triage-2026-08-13.mjs` 与 `docs/content-triage-2026-08-13.md`；OSS 图由 `scripts/purge-triaged-project-assets.mjs` 另跑（默认 dry-run；库行已删时改为列举 OSS 前缀）。应用使用 `pnpm db:push`。
...
- `20260825130000_content_classification_ranking_visibility.sql` — 热门/本周热门 ranking RPC 与阶段二公开可见性对齐，修正过滤后的总数与分页；内容分级相关迁移已通过 `pnpm db:push` 执行，当前仅公开三轴展示开启，发布门禁仍保持关闭
- `20260826161222_content_classification_public_v1.sql` — 受控开启公开 reviewed 三轴 DTO，并新增 `set_content_classification_public_v1` RPC；写入 `public_enabled/public_disabled` 审计事件，但不改变 `enforcement_enabled`
...
- `20260819100000_unified_project_journeys.sql` — 统一普通项目与 PBL 的 `project_journeys` / `project_journey_records`；一用户一来源只允许一个 active Journey，过程默认私密、公开后审核，旧项目探索/PBL 阶段与作品表保留兼容关联；应用迁移使用 `pnpm db:push`，不要使用 `supabase db push`
...
- `20260823100000_store_external_checkout.sql` — 为 `store_products` 增加 `checkout_mode`、`external_channel`、`external_url`，支持淘宝外部结算商品卡片；默认值保持既有商品为站内模式，应用迁移使用 `pnpm db:push`，不要使用 `supabase db push`
- `20260823110000_store_context_keys.sql` — 为商品增加 `context_keys`，把淘宝材料包挂到课程/课时/项目详情页；应用迁移使用 `pnpm db:push`，不要使用 `supabase db push`
...
- `20260828140000_fill_project_reflections.sql` — 为 214 个已审核公开项目填充唯一的一句话 `reflection`；使用固定项目 ID 白名单、空值保护、事务与 `service_role` 执行，迁移已执行后核验公开项目为 214/214 有结论。迁移文件已执行后保持不可变；未来批量回填应另建迁移并同时加公开状态条件。应用迁移使用 `pnpm db:push`，不要改写已执行迁移文件。
...
`profiles.age_confirmed_at` 仅由社区互动确认流程写入，当前只控制公开作品/评论等内容写入；私信仍通过同一 `current_user_can_interact()` 触发器检查账号限制，但不要求该字段。`profiles.interaction_restricted` 由后台限制流程使用；固定 XP 事件通过 service-role-only `award_xp_once()` 原子写入 `xp_logs` 与 `profiles.xp`，项目评论每日最多计入 50 XP。
...
| `set-content-classification-public-v1.mjs` | 通过受控 RPC 开关公开三轴 DTO；必须显式传 `--enable/--disable`、原因和管理员身份，不接入构建或页面请求 |
...
| `purge-triaged-project-assets.mjs` | 2026-08-13 内容分诊：按 `scripts/lib/content-triage-2026-08-13.mjs` 的 105 个项目 ID 清理 Aliyun OSS 上该项目自己的 `projects/generated/` 与 `projects/steps/` 对象，并删除本地对应文件、从封面清单去掉已删 ID；库行还在时从 `projects` / `project_steps` / `comments` 收集 URL，库行已删时列举 OSS 前缀按文件名里的项目 ID 匹配；默认 dry-run，`--execute` 才删除 |
...
- `playwright.config.ts` / `playwright.integration.config.ts` / `playwright.scratch-host.config.ts` — Playwright 配置；Scratch host 套件与主站 E2E 隔离，避免依赖 Next、数据库或登录；GitHub CI 通过 `PLAYWRIGHT_USE_SYSTEM_CHROME=1` 使用 runner 预装的 Chrome，开发机默认仍使用 Playwright Chromium
...
- `deploy/auto-interactions-worker.mjs` — 自动互动队列 Docker worker（可在启动时按 `AUTO_INTERACTION_BACKFILL_*` 低比例补偿历史项目；随后按 `AUTO_INTERACTION_WORKER_INTERVAL_SECONDS` 周期 POST `/api/internal/auto-interactions/run`）。`AUTO_REPLY_RATE` / `AUTO_INTERACTION_BACKFILL_REPLY_RATE` 随拟人化短评一起删除，只剩点赞与收藏比例
- `Dockerfile` — 生产镜像构建；基于 Node 22 构建并运行 Next standalone 与自动互动 worker 脚本；builder 复用 deps 阶段的 pnpm 与 `node_modules`，避免重复初始化和复制依赖
- `.github/workflows/ci.yml` — CI：Lint + TypeScript + Vitest + Playwright；PR 仍跑完整 `pnpm build`，主分支 push 由 Release 的 Docker 构建覆盖，避免同一次提交重复打包；GitHub runner 使用预装 Chrome，不再每次下载 Playwright 浏览器或安装系统依赖；同一分支连续 push 会取消过时 CI 以节省分钟；本地只在 `pre-commit` 跑 lint 和改动相关的用例，全量单测以此为唯一门槛，推送后用 `pnpm ci:watch` 看结论
- `.github/workflows/release.yml` — Release：用 Docker Buildx 的 GitHub Actions 缓存构建 Docker 镜像（复用依赖层，避免每次从零安装），再渲染/同步 Nginx 配置、同步 compose 文件并 SSH 部署；`.dockerignore` 排除本地工具缓存、测试/文档和构建临时目录，减少 Docker 构建上下文；Nginx 默认写入 `/etc/nginx/sites-available/steam-app` 并维护 `/etc/nginx/sites-enabled/steam-app`，默认域名 `steamx.cc www.steamx.cc`；部署后可用 `BAIDU_PUSH_TOKEN` secret 调 `scripts/submit-baidu-urls.mjs` 主动推送 sitemap 中的公开 URL（未配置时跳过且不阻塞发布）；爬虫访问改由 `robots.ts` 约束，不再在 Nginx 层按 UA 拒绝 GPTBot；站点参数可用 `NGINX_SERVER_NAME`、`NGINX_SSL_CERTIFICATE`、`NGINX_SSL_CERTIFICATE_KEY`、`NGINX_SITE_PATH`、`NGINX_SITE_ENABLED_PATH` 覆盖，若线上使用 `/etc/nginx/conf.d/*.conf` 可将两个 path 变量设为同一路径以跳过 `sites-enabled` symlink
