...
- `app/manifest.ts` / `app/robots.ts` / `app/sitemap.ts` / `app/llms.txt/route.ts` — PWA & SEO；`robots.ts` 为 Baiduspider、GPTBot 及其他常见大模型爬虫明确放行公开页，并与 `User-Agent: *` 共用 `/api/` `/admin/` `/login` 等私有路径 Disallow；Sitemap 指向 `https://www.steamx.cc/sitemap.xml`，使用无请求 cookie 的公开 Supabase 客户端并按小时 ISR 刷新，覆盖静态页 + 项目 + 物种 + 观察 + **课程 / 课时 / 有模型课时的零件清单页**（300 多节免费课过去完全不在 sitemap 里，等于白做），课程与课时详情页各自带 `keywords`、成品图 OG、Course/LearningResource JSON-LD 与面包屑；物种图鉴补充真实 `<h1>`、服务端首屏导语和详情 Taxon/WebPage JSON-LD；根布局标记 `lang=zh-CN` 与 `applicable-device=pc,mobile`；首页显式保留一次 `内容 | STEAMX · 史迪姆`，其他页面由全局模板追加品牌，避免标题重复；`/llms.txt` 返回 `text/plain` 站点说明
- `proxy.ts` — Next.js 16 Proxy 入口：仅当 Host 为 `steamx.cc` 时 301 到对应的 `www.steamx.cc` 路径（含 query），不改写 localhost / 预览 / tunnel / 未来第二域名；`/robots.txt` 与 `/sitemap.xml` 直接放行且不写匿名推荐 cookie，保证搜索引擎发现文件稳定；其他请求补种匿名推荐 `rec_viewer` cookie（替代已废弃的 `middleware.ts`），并把当前 pathname/search 写入 `x-steam-pathname` / `x-steam-search`，供未传 `next` 的 `requirePageUser()` 拼登录回跳
...
- `json-ld.ts` — 首页 WebSite/Organization/SearchAction、项目页 Article（有步骤时再加 HowTo）、课程 Course、课时 LearningResource、物种 Taxon/WebPage、面包屑 JSON-LD；只使用已有标题/描述/步骤/封面与公开观察链接，由 `components/seo/json-ld.tsx` 输出 `<script type="application/ld+json">`
- `llms-txt.ts` / `robots-policy.ts` — `/llms.txt` 正文与爬虫 Disallow / 大模型 UA 列表
- 公开 SEO 路由规则：`/playground` 首页、18 个游戏页及扫雷课程页使用独立 title/description/canonical；PBL 详情由 `app/pbl/[id]/layout.tsx` 服务端校验 active/ended 挑战并生成动态 metadata，不存在或不可见挑战返回真正 404；挑战作品提交页明确 `noindex`；sitemap 不提交带筛选 query 的物种页，静态 URL 不伪造动态 `lastmod`，零件清单仅在部署产物存在对应 LDraw 模型时收录
...
- `weekly-plan.ts` — 个人主页「本周探索计划」纯规则生成：3-5 步学习路径，聚合并保留当前周已完成时间线（最多 3 条）、PBL 阶段、在学课程、探索中项目、STEAM 雷达、自然观察与新手引导；**新手引导已并进这里，个人主页只保留这一个待办概念**（原先桌面端同屏挂着 `PrimaryPlanCard` / `ProfileStarterHub` / `GrowthTasksPanel` 三块说同一件事的面板，后两块已删；未毕业时在计划卡头部显示一行「新手引导 N/5」保住毕业进度感）。起步周计划的三步直接由新手引导任务定义生成，让「这周要做的事」和「能领的奖励」是同一批对象；同一摘要供小迪 profile 场景解读
...
| `lib/auth/` | `server.ts`, `login-redirect.ts` | 服务端认证辅助；`requirePageUser()` 默认按 proxy 转发的当前路径生成 `/login?next=...`，受保护分区的 layout 不要再传写死的 `next`（会把 `/messages/[userId]`、`/profile/timeline`、`/share?edit=` 这类深链接压成分区首页）；`/settings/about` 视为公开设置页 |
...
- 各目录内 `*.test.ts(x)` — 就近放置的单元测试；项目探索记录组件覆盖作品详情留言引导、预览卡和完整记录流的最终作品直达作品页、探索记录不误显作品入口，以及作品留言/多级回复的独立举报与自有内容隐藏举报；SEO 相关覆盖 `lib/seo/*`、`app/robots.ts`、`/llms.txt`、全站页脚 ICP、`proxy.ts` 仅对 `steamx.cc` 做 301，以及 `deploy/nginx.conf` 不再拦截 GPTBot
