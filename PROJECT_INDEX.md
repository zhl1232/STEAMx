...
- `.github/workflows/ci.yml` — CI：Lint + TypeScript + Vitest + Playwright；PR 仍跑完整 `pnpm build`，主分支 push 由 Release 的 Docker 构建覆盖，避免同一次提交重复打包；同一分支连续 push 会取消过时 CI 以节省分钟；本地只在 `pre-commit` 跑 lint 和改动相关的用例，全量单测以此为唯一门槛，推送后用 `pnpm ci:watch` 看结论
