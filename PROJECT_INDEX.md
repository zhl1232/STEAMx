...
- `app/manifest.ts` / `app/robots.ts` / `app/sitemap.ts` / `app/llms.txt/route.ts` — PWA & SEO；`robots.ts` 允许 GPTBot 及其他常见大模型爬虫抓取公开页，并与 `User-Agent: *` 共用 `/api/` `/admin/` `/login` 等私有路径 Disallow；Sitemap 指向 `https://www.steamx.cc/sitemap.xml`，覆盖静态页 + 项目 + 物种 + 观察 + **课程 / 课时 / 有模型课时的零件清单页**（300 多节免费课过去完全不在 sitemap 里，等于白做），课程与课时详情页各自带 `keywords` 与成品图 OG；`/llms.txt` 返回 `text/plain` 站点说明
