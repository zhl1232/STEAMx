...
| `/profile` | `app/profile/page.tsx` | 个人主页 — 桌面首屏按「个人 Hero → 本周计划 / 今日行动 → 能力雷达与作品观察摘要」组织，普通桌面主体摘要在宽版卡片内左右并列，大桌面再将经验等级与学习打卡（每日登录流水与学习行为统一计算）放入 400px 右栏（新手引导不再单开面板，已并进本周探索计划，见 `lib/profile/weekly-plan.ts`）；移动端采用现代 Bento 便当盒美学重构为三大高凝聚层级：1.「Hero 个人画像中枢」融合身份称号、徽章阵列、经验条、4 项资产统计与 4 个高频快捷入口（内容/消息/钱包/商店）；2.「探索与成长行动中心」将每日打卡紧凑条与本周探索计划无缝合体（标题栏集成手风琴折叠切换器，任务顺序绝对稳定不跳变）；3.「成长画像与观察成果」将 STEAM 五维能力雷达与自然观察图鉴进度合二为一，突破指南转为标题右上角轻量弹窗，彻底消灭碎片化孤岛卡片堆叠，大幅提升信息密度与视觉呼吸感；移动端标题栏只保留设置，公开主页入口由头像/资料区承担，避免与资料编辑重复；首页作品统计与摘要读取统一作品，内容库区分「我的作品」与用户创建的「发布的项目」；子路由 `library/`、`timeline/`、`likes/`、`followers/`、`following/` |
