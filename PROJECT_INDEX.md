...
- 本次内容分诊硬删除：`20260813090000_hard_delete_triaged_projects.sql` 按站长 2026-08-13 审定的 105 个项目 ID 永久删除（不是下架），并先清子表/关联行；权威名单与保留 ID 见 `scripts/lib/content-triage-2026-08-13.mjs` 与 `docs/content-triage-2026-08-13.md`；OSS 图由 `scripts/purge-triaged-project-assets.mjs` 另跑（默认 dry-run）。应用使用 `pnpm db:push`，不要从本云端环境对生产执行删除。
