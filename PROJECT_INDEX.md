...
| `/` | `app/page.tsx` | 首页 — Hero 保留原有 STEAM 探索文案与 `/explore`、`/create` 入口；移动端 Hero 下方快捷入口的第一页为「新手推荐 / 挑一节积木课」，其后保持自然观察与排行榜；桌面端在「从这里开始」保留新手推荐并展示分类入口；主体不再重复探索页热门项目，改为统一作品「大家的新作品」与最近公开观察「自然新发现」，桌面双栏、移动端顺序滚动；继续保留移动端自然观察/排行榜快捷入口、桌面社区动态与本周挑战（挑战卡读取当前 `active` PBL，优先限时挑战，卡片进入 `/pbl/[id]`，「查看全部」进入 `/create`） |
...
| profile | `api/profile/` | 个人资料摘要、新手引导、学习打卡、本周探索计划（聚合 PBL 阶段/课程/自然观察/雷达等信号）；`works` 返回当前用户的四类作品（项目 / 课时 / 挑战 / 自然观察）；`growth-tasks/sync` 与 `weekly-plan` 都会读取 `profiles.bio` 并调用 `get_user_stats_summary` 计算成长任务进度 |
...
| `gamification/` | 15 | 徽章图标/画廊、等级进度、排行榜、成就 Toast、每日登录同步（登录用户首页也挂载，临时失败自动重试）、观察游戏化同步；图鉴按系列只露最高档，点开 `badge-detail-dialog` 看档位清单与进度，昆虫钻石未解锁不出现；徽章解锁先写入轻量 `badge-unlock-store`，`badge-unlock-overlay-mount` 只有队列非空时才加载 confetti/Framer Motion 庆祝层；单枚全屏庆祝可点跳过/超时关闭，同批 ≥2 枚改为汇总网格一次看全并链到 `/profile` 图鉴 |
| `moderator/` | 2 | 审核员申请表单 |
...
- `badges.ts` — 全部徽章定义（独立/阶梯/系列）；阶梯系列用 `tierNames` 独立成就名，档位可用 `BADGE_TIER_LABELS` 作说明文本，昆虫观察额外有 `diamond`；图鉴与公开主页经 `getSeriesDisplayBadge()` 每系列只露最高档，未解锁钻石不出现；STEAM / 项目完成 / 初级探索者按项目+课时/观察计数；社区 `social` 只数留言+回复；自然观察保留条数 `bird_observer` 与种数 `species_collector`，并增加常见鸟 / 进阶鸟 / 稀有鸟与昆虫观察（手册 D/C/B/A/S）；游乐场 `playground_explorer` 门槛 3/8/13/18；内测徽章 `beta_tester` 由 `GRANT_BETA_TESTER_BADGE` 控制
- `species-difficulty.ts` — 站内 133 种鸟的常见/进阶/稀有对照，以及昆虫手册八套九宫格与 S 级锹甲/大蚕蛾/大步甲/北京神物名录
