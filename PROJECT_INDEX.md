...
| `/profile` | `app/profile/page.tsx` | 个人主页 — 桌面首屏按「个人 Hero → 本周计划 / 今日行动 → 能力雷达与作品观察摘要」组织，普通桌面主体摘要在宽版卡片内左右并列，大桌面再将经验等级与学习打卡放入 400px 右栏（新手引导不再单开面板，已并进本周探索计划，见 `lib/profile/weekly-plan.ts`）；移动端保留 4 个高频入口（内容、消息、钱包、商店）并继续展示本周探索计划、STEAM 雷达、自然观察进度和徽章，徽章图鉴弹窗按系列平铺展示，阶梯系列只显示当前最高档，独立徽章单枚展示，并支持「全部 / 已拥有 / 未解锁」筛选，头像等级标记独立放在头像下方且头像框装饰不被 Hero 裁切；首页作品统计与摘要读取统一作品，内容库区分「我的作品」与用户创建的「发布的项目」；子路由 `library/`、`timeline/`、`likes/`、`followers/`、`following/` |
...
| `/users/[id]` | `app/users/[id]/` | 其他用户的公开主页，下方 Tab 栏聚焦公开创作内容（区分「作品」与「项目」两个 Tab）；公开主页头像统一使用 `UserAvatar` 并展示用户已装备的头像框与昵称颜色，等级徽章贴角整合于头像正下方；用户名旁展示佩戴/自动推导的个性化成就称号微胶囊，个人信息区展示等大圆形勋章阵列（前 4 枚 32px 纯金属勋章 + 第 5 个同尺寸圆形「+N」入口，绝对单行整齐排列，自动推导时按系列去重只取各系列最高品质），点击即可呼出全量「成就徽章图鉴」大弹窗浏览全部系列与进阶进度；徽章详情弹窗支持一键“佩戴称号”与“设为主页展出”；首屏使用资料页探索背景素材配柔和渐变遮罩，统计栏采用独立微卡片半透明质感，移动端使用透明页头保留返回与标题，操作区采用关注+私信双主按钮并由右上角下拉菜单收纳屏蔽与分享，Tab 选中态统一使用品牌蓝，移动端隐藏全局小迪入口避免遮挡公开内容；非法 UUID 在查库前按未找到处理，避免 Postgres `22P02` 打成 500 |
...
| profile | `api/profile/` | 个人资料摘要、新手引导、学习打卡、本周探索计划（聚合 PBL 阶段/课程/自然观察/雷达等信号）；`equipped-honor` 支持一键佩戴/卸下称号（`equipped_title`）与配置主页精选徽章展台（`featured_badge_ids`，最多 3 枚）；`works` 返回当前用户的四类作品（项目 / 课时 / 挑战 / 自然观察）；`growth-tasks/sync` 与 `weekly-plan` 都会读取 `profiles.bio` 并调用 `get_user_stats_summary` 计算成长任务进度 |
...
| `gamification/` | 15 | 徽章图标/画廊、等级进度、排行榜、成就 Toast、每日登录同步（登录用户首页也挂载，临时失败自动重试）、观察游戏化同步；图鉴按系列只露最高档，点开 `badge-detail-dialog` 看档位清单与进度，昆虫钻石未解锁不出现；自然观察徽章 PNG 使用透明背景，资源 URL 带版本参数避免线上图片优化缓存继续返回旧黑底图；徽章解锁先写入轻量 `badge-unlock-store`，`badge-unlock-overlay-mount` 只有队列非空时才加载 confetti/Framer Motion 庆祝层；单枚全屏庆祝可点跳过/超时关闭，同批 ≥2 枚改为汇总网格一次看全并链到 `/profile` 图鉴 |
| `moderator/` | 2 | 审核员申请表单 |
