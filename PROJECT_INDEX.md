...
| 子目录 | 文件数 | 职责 |
|--------|--------|------|
| `bird-observation/` | 13 | 观察提交表单、照片上传、Leaflet 地图选点/热点预览、观察卡片、物种热点面板、物种统计面板（无观察记录时隐藏）、评论区；地图引擎从本站静态资源加载，底图继续使用国内高德瓦片；照片上传先从原始文件读取 EXIF 拍摄时间与 GPS，再上传客户端压缩副本，自动回填使用首个可用拍摄时间与首张带 GPS 的照片，不比较多张补充照片的位置差异，也不让后续照片覆盖已有地点；照片均无 GPS、GPS 转地图坐标失败或反查不到地点名时，会在地点步骤提示手动搜索/选点；观察详情轮播复用共享 `SwipeablePhotoViewer` / `ImageLightbox`，当前缩略图使用 eager 加载、其他缩略图保持 lazy，保证切换后的选中缩略图立即可见；观察详情 AI 鉴定头像使用 `public/xiaodi-ai/idle-0.webp` 小迪静帧 |
...
| `works/` | 4 | 统一作品 UI：`work-card-grid` 为探索页、课程与个人主页提供响应式作品卡片；`work-detail` 展示媒体、来源、作者、评论和审核状态，项目探索时间线最多展示最近 50 条并在截断时标出总数，点赞/投币与作者信息同行，作品留言的回复/举报控件保留至少 44px 触控命中区，投过币的作品会在投币按钮上显示琥珀色已支持状态，空创作说明不占位，并仅向作品作者开放分享交互；`work-image-gallery` 提供预览、缩略图跳转和全屏看图，时间线内按竖图比例居中展示并裁切在卡片内，避免横框留白溢出；`share-work-dialog` 内的 `ShareCardDialog` 统一为作品和项目生成固定画布高清卡片与高纠错二维码，并提供系统分享、保存图片和复制链接 |
