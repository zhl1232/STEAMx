...
| `/project/[id]` | `app/project/[id]/page.tsx` | 项目详情 — 步骤、材料清单、点赞/收藏、作品与探索记录、打赏、项目举报；不再提供项目评论，留言、提问和建议统一进入具体作品；项目详情预览中的最终作品卡直接进入作品详情并显示「查看作品并留言」，没有作品时引导先上传；「全部探索记录」页按 `exploration_id` 将同次探索合并为堆叠卡片，每组只加载一张代表图并汇总步骤/互动数，避免每一步重复占满记录流；记录类型筛选只隐藏展示帖子但保留终稿状态，超过 48 条时明确标注当前列表统计；完成组优先进入终稿，未完成组也可从最新步骤进入完整探索详情，作者可继续选择任一步作为完成作品；移动端顶部操作区为已登录的非作者提供举报入口，桌面端项目操作移到详情头部；历史课程背书项目会重定向到对应课时作品区 |
...
| `works/` | 4 | 统一作品 UI：`work-card-grid` 为探索页、课程与个人主页提供响应式作品卡片；`work-detail` 展示媒体、来源、作者、评论和审核状态，项目探索时间线最多展示最近 50 条并在截断时标出总数，点赞/投币与作者信息同行，作品留言的回复/举报控件保留至少 44px 触控命中区，投过币的作品会在投币按钮上显示琥珀色已支持状态，空创作说明不占位，并仅向作品作者开放分享交互；`share-work-dialog` 以固定画布生成高清作品卡片与高纠错二维码，并提供系统分享、保存图片和复制链接 |
