...
| `ldraw-models/` | 大颗粒 LDraw 源模型与生成脚本目录；逐课生成器、assembly JSON、`.ldr`、BOM/report 放在这里，新增或修改模型后统一跑 `.agents/skills/image-to-ldraw/scripts/resolve-parts.mjs`、`validate-assembly.mjs`、必要时 `assembly-to-ldraw.mjs`，再用 `pack-ldraw-model.mjs` 打包为 `public/courses/ldraw/*.mpd` |
...
| `pack-ldraw-model.mjs` | 把 `scripts/ldraw-models/*.ldr` 递归抓取依赖打包成单个自托管 `.mpd` + `LDConfig.ldr` 到 `public/courses/ldraw/`（大颗粒积木 3D 课用，零件库 CC BY / CCAL）；打包时优先使用 `scripts/ldraw-models/parts|p|models/` 本地自定义件，再复用已提交 `.mpd` 内联依赖作为缓存，最后联网抓取；用法 `node scripts/pack-ldraw-model.mjs <source.ldr> <outName>` |
